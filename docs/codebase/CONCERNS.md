...
| high | Console routes import an artifact feature directory absent from the current tree | `src/Hosting/ElsaControl.Console/src/app/routes.tsx`; no `src/Hosting/ElsaControl.Console/src/features/artifacts/` | Current frontend typecheck/build is expected to fail and prior completion claims are stale | Restore or deliberately remove the feature and add explicit frontend CI |
