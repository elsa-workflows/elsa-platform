...
|----------|---------|----------|--------|------------------|
| high | No Azure provider currently realizes an Elsa desired configuration into a working managed Elsa endpoint | AppHost/`infra/` deploy only Elsa Control; console `runtimes` route is a placeholder | Blocks deployment proof and Elsa Cloud walking skeleton | Run an Azure-provider spike and implement one thin vertical slice |
| high | Console routes import an artifact feature directory absent from the current tree | `src/Hosting/ElsaControl.Console/src/app/routes.tsx`; no `src/features/artifacts/` | Current frontend typecheck/build is expected to fail and prior completion claims are stale | Restore or deliberately remove the feature and add explicit frontend CI |
