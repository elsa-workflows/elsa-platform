...
`valence-works/elsa-control`. The existing repository object was transferred;
Git history was not rewritten or recreated.

The product name changed from Elsa Platform to Elsa Control. Product-owned
.NET projects, assemblies, packages, and namespaces changed from
`Elsa.Platform.*` to `ElsaControl.*`. Product-owned infrastructure and
configuration use the `elsa-control` slug.
...
to the Elsa Control Commercial License. Earlier revisions remain governed by
...
- `dotnet restore ElsaControl.sln` passed.
...
- Both owned NuGet packages packed locally without publishing. Inspection
  confirmed the proprietary `LICENSE` and only `ElsaControl.*` owned
...
- The API container image built locally as
  `elsa-control-api:migration-validation` without being pushed. The clean
