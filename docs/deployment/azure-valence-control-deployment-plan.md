...
scripts/deploy-azure-valence-control.sh \
  --environment prod \
...
| --- | --- | --- |
| `development` | `dev` | `rg-valence-control-dev` | Lower SKU, disposable data. |
| `test` | `test` | `rg-valence-control-test` | Production-like config for release validation. |
| `production` | `production` or `prod` | `rg-valence-control-prod` | Strong secrets, backups, access review. |
...
scripts/bootstrap-github-azure.sh \
  ...
  --resource-group rg-valence-control-dev \
  --location westeurope
```
