...
Tracking issue: [#50](https://github.com/valence-works/elsa-control/issues/50)

## Purpose

This document defines the minimum trust rules for Elsa Control integrations that submit or consume deployment artifacts.
...
Runtime command sync uses outbound runtime-to-Elsa Control calls for poll, claim, heartbeat, progress, completion, failure, and rejection.
...
1. Register the workflow engine in Elsa Control with endpoint metadata, capability metadata, and a credential reference.
2. Create a runtime integration identity in the selected identity provider or secret provider.
3. Store the identity's secret material outside Elsa Control.
4. Configure the runtime applier host with `ElsaControl:Auth:CredentialReference`.
...
- Use stable secret references in Elsa Control-facing configuration, for example `kv://claims/runtime-sync`.
...
- If the runtime can no longer authenticate to Elsa Control, stop applying commands and keep polling disabled until credentials are repaired.
...
- Keep payload access credentials outside Elsa Control catalog tables and diagnostics.
...
- Authenticate or otherwise trust the Elsa Control sender through host-owned policy.
...
4. Enable `Deployment:WebhookDispatch:Enabled` in Elsa Control.
