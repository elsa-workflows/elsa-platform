...
Tracking issue: [#50](https://github.com/elsa-workflows/elsa-platform/issues/50)

## Purpose

This document defines the minimum trust rules for Elsa Platform integrations that submit or consume deployment artifacts.
...
Runtime command sync uses outbound runtime-to-Platform calls for poll, claim, heartbeat, progress, completion, failure, and rejection.
...
1. Register the workflow engine in Platform with endpoint metadata, capability metadata, and a credential reference.
2. Create a runtime integration identity in the selected identity provider or secret provider.
3. Store the identity's secret material outside Platform.
4. Configure the runtime applier host with `ElsaPlatform:Auth:CredentialReference`.
...
- Use stable secret references in Platform-facing configuration, for example `kv://claims/runtime-sync`.
...
- If the runtime can no longer authenticate to Platform, stop applying commands and keep polling disabled until credentials are repaired.
...
- Keep payload access credentials outside Platform catalog tables and diagnostics.
...
- Authenticate or otherwise trust the Platform sender through host-owned policy.
...
4. Enable `Deployment:WebhookDispatch:Enabled` in Platform.
