# Valence Control Phased Implementation Strategy
...
This document captures the original CLI-first deployment-platform strategy. The current artifact-driven control-plane baseline, including Studio **Submit to Valence Control**, runtime command sync, workflow artifact runtime application, and artifact-backed promotion, is maintained in [Valence Control Artifact Deployment Execution Plan](valence-control-artifact-deployment-execution-plan.md). Use that execution plan for current implementation sequencing.

## 1. Product Framing

The Valence Control is the deployment control plane for Elsa-based systems. It packages, validates, previews, applies, and records declarative desired state for runtime configuration and workflow assets across environments.
...
valence-control/
  ValenceControl.sln
  docs/
    valence-control-deployment-phased-strategy.md
    adr/
  src/
  ...
  tests/
    ...
    ValenceControl.Deployment.Cli.Tests/
    ValenceControl.Deployment.Api.Tests/
...
```text
src/
  ValenceControl.Deployment.GitOps/
  ValenceControl.Deployment.Operator/
  ValenceControl.Deployment.Kubernetes/
  ValenceControl.Deployment.Oci/
  ValenceControl.Deployment.Signing/
  ValenceControl.Deployment.Policies/
```

### Elsa Core Versus Valence Control Ownership
...
`valence-control` should own platform behavior:
...
- First foundation slice: `src/ValenceControl.Deployment.Abstractions` and `tests/ValenceControl.Deployment.Abstractions.Tests`.
...
- First manifest slice: `src/ValenceControl.Deployment.Manifest` and `tests/ValenceControl.Deployment.Manifest.Tests`.
...
| Capability | Phase 1: Foundation | Phase 2: Enterprise | Phase 3: Valence Control Engineering |
| --- | --- | --- | --- |
...
## 8. Phase 3 Valence Control Engineering Scope
...
- Valence Control-wide deployment dashboards.
...
| Valence Control leaks into runtime state | Unsafe reconciliation and data loss | Hard-code non-goals into docs and tests; avoid handlers for instances, bookmarks, logs, locks, queues, and execution state |
...
The package descriptor validation contract lives in `ValenceControl.PackageCatalog.Abstractions` so `ValenceControl.Deployment.*` packages can reference it directly while remaining independent from Valence Control API, UI, persistence, migrations, and source-provider internals.
...
Start with `apiVersion: valence-control/v1alpha1` and `kind: EnvironmentManifest`.
...
1. Define Phase 1 deployment contracts in `ValenceControl.Deployment.Abstractions`
...
| ADR-0002 | Phase 1 | Initial package structure and `elsa-core` versus `valence-control` ownership |
...
```markdown
I converted the PRD into a phased implementation strategy in the new `valence-control` repository:

<link to docs/valence-control-deployment-phased-strategy.md>
