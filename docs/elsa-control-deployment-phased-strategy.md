# Elsa Control Phased Implementation Strategy
...
This document captures the original CLI-first deployment-platform strategy. The current artifact-driven control-plane baseline, including Studio **Submit to Elsa Control**, runtime command sync, workflow artifact runtime application, and artifact-backed promotion, is maintained in [Elsa Control Artifact Deployment Execution Plan](elsa-control-artifact-deployment-execution-plan.md). Use that execution plan for current implementation sequencing.

## 1. Product Framing

The Elsa Control is the deployment control plane for Elsa-based systems. It packages, validates, previews, applies, and records declarative desired state for runtime configuration and workflow assets across environments.
...
elsa-control/
  ElsaControl.sln
  docs/
    elsa-control-deployment-phased-strategy.md
    adr/
  src/
  ...
  tests/
    ...
    ElsaControl.Deployment.Cli.Tests/
    ElsaControl.Deployment.Api.Tests/
...
```text
src/
  ElsaControl.Deployment.GitOps/
  ElsaControl.Deployment.Operator/
  ElsaControl.Deployment.Kubernetes/
  ElsaControl.Deployment.Oci/
  ElsaControl.Deployment.Signing/
  ElsaControl.Deployment.Policies/
```

### Elsa Core Versus Elsa Control Ownership
...
`elsa-control` should own platform behavior:
...
- First foundation slice: `src/ElsaControl.Deployment.Abstractions` and `tests/ElsaControl.Deployment.Abstractions.Tests`.
...
- First manifest slice: `src/ElsaControl.Deployment.Manifest` and `tests/ElsaControl.Deployment.Manifest.Tests`.
...
| Capability | Phase 1: Foundation | Phase 2: Enterprise | Phase 3: Elsa Control Engineering |
| --- | --- | --- | --- |
...
## 8. Phase 3 Elsa Control Engineering Scope
...
- Elsa Control-wide deployment dashboards.
...
| Elsa Control leaks into runtime state | Unsafe reconciliation and data loss | Hard-code non-goals into docs and tests; avoid handlers for instances, bookmarks, logs, locks, queues, and execution state |
...
The package descriptor validation contract lives in `ElsaControl.PackageCatalog.Abstractions` so `ElsaControl.Deployment.*` packages can reference it directly while remaining independent from Elsa Control API, UI, persistence, migrations, and source-provider internals.
...
Start with `apiVersion: elsa-control/v1alpha1` and `kind: EnvironmentManifest`.
...
1. Define Phase 1 deployment contracts in `ElsaControl.Deployment.Abstractions`
...
| ADR-0002 | Phase 1 | Initial package structure and `elsa-core` versus `elsa-control` ownership |
...
```markdown
I converted the PRD into a phased implementation strategy in the new `elsa-control` repository:

<link to docs/elsa-control-deployment-phased-strategy.md>
