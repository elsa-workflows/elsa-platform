# Platform Artifact Workflow E2E Smoke
...
```text
Elsa Studio Submit to Platform
...
- `elsa-platform/main` includes specs `024` through `030`.
- `elsa-studio/main` includes the `Elsa.Studio.PlatformIntegration` module and neutral workflow action zones.
- A Platform workspace exists with permission to register artifacts, manage desired state, preview promotion, deploy, and roll back.
- A Platform environment exists with at least one registered runtime target advertising `workflow-definition.apply`.
- A runtime-integrated Elsa Workflows app can poll Platform runtime commands and apply `elsa.workflow-definition` artifacts.
- The Studio integration and runtime applier are configured with Platform endpoint, workspace, authentication, and approved payload transport settings.

## Smoke Sequence

1. Author or open a workflow definition in Elsa Studio.
2. Choose **Submit to Platform** from the workflow editor toolbar.
3. Verify the Platform artifact registry contains an `elsa.workflow-definition` artifact with:
...
8. Verify Platform creates a deployment run and runtime command that references the artifact by safe identity/digest and does not embed raw workflow content.
...
10. Verify the runtime applier:
    ...
    - reports progress and completion to Platform.
11. Verify Platform deployment history shows command progress, completion, runtime reference, observed digest, and safe diagnostics.
...
- Submit with invalid Platform credentials and verify Studio reports a safe unauthorized result.
- Submit unsafe display metadata and verify Platform rejects or sanitizes it without storing unsafe metadata.
