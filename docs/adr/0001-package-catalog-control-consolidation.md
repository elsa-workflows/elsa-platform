# ADR-0001: Consolidate Package Catalog Into Elsa Control
...
The Elsa Control roadmap needs package descriptor validation, package
...
Move Package Catalog into `elsa-control` as a first-class sibling subsystem to Deployment.
...
Runtime Builder should be extracted as its own Elsa Control subsystem when normalizing the imported catalog code. It owns builder intent, runtime image metadata, bundle generation, server-side planning, deployment template rendering, and saved runtime configurations.

## Consequences

Positive:

- One Elsa Control repository owns deployment and package governance.
- Package descriptor validation can share catalog compatibility and approval semantics.
- Package manifests remain a shared Elsa Control contract.
- Runtime Builder backend capabilities get a clear home instead of remaining mixed into catalog core.
- The old catalog repository can be deprecated once the Elsa Control subsystem is usable.

Tradeoffs:

- The Elsa Control repository becomes larger earlier.
...
- Keep Elsa Control's Spec Kit infrastructure active.
- Use `specs/001-elsa-control-package-catalog-consolidation/tasks.md` as the progress tracker.
