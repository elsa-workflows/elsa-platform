...
**World 1 — `src/Elsa.Platform.Deployment.Engine` (the typed reconciler).**
...
- The `Elsa.Platform.Deployment.Engine` project is referenced **only** by
...
- `IResourceValidator` and `IResourceStateReader`
  (`src/Elsa.Platform.Deployment.Abstractions/Resources/`) have **zero**
...
1. **Land the typed desired-state reader (no behavior change).**
   ...
   `Elsa.Platform.Deployment.Core` reusing the Abstractions model. Cover it with
   tests mirroring the existing `ParseRecords` / `ParseArtifactReferences` cases.
   Nothing consumes it yet.
   *Files:* new reader in `src/Elsa.Platform.Deployment.Core/Workspace/`;
   `src/Elsa.Platform.Deployment.Abstractions/Resources/*`.
...
2. **Route `DeploymentDeployabilityService` through the typed reader.**
   ...
   *Files:* `src/Elsa.Platform.Deployment.Core/Workspace/DeploymentDeployabilityService.cs`.
...
3. **Route `DeploymentValidationService` through the typed reader** and replace
   ...
   *Files:* `src/Elsa.Platform.Deployment.Core/Workspace/DeploymentValidationService.cs`;
   `src/Elsa.Platform.Deployment.Abstractions/Plans/*`.
...
4. **Replace substring blocker classification with typed diagnostics.**
   ...
   *Files:* `DeploymentDeployabilityService.cs`;
   `src/Elsa.Platform.Deployment.Abstractions/Diagnostics/*`;
   `src/Elsa.Platform.Deployment.Engine/DeploymentEngineDiagnosticCodes.cs`.
...
5. **Introduce the analytical core as a referenced library.**
   ...
   *Files:* `src/Elsa.Platform.Deployment.Engine/*.csproj` references;
   `src/Elsa.Platform.Deployment.Core/*.csproj`;
   `tests/Elsa.Platform.Deployment.Engine.Tests/*`.
...
6. **Delete dead in-process-execution abstractions.**
   ...
   *Files:* `src/Elsa.Platform.Deployment.Abstractions/Resources/IResourceValidator.cs`,
   `IResourceStateReader.cs`, `IResourceHandler.cs`;
   `src/Elsa.Platform.Deployment.Abstractions/Targets/*`;
   `src/Elsa.Platform.Deployment.Engine/DeploymentEngine.cs` (drop/relocate the
...
7. **Implement spec 037 `ConfigurationBinding` as the first production
   ...
   *Files:* new handler in `src/Elsa.Platform.Deployment.Core/`;
