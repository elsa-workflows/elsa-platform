...
Cleanup verifies exact proof ownership tags. When external deployment provenance is present, it recomputes and requires the exact deterministic deployment name plus a valid registry-scoped role-assignment output. A partial foundation without that deployment tag uses the narrowly scoped fallback: only AcrPull assignments enumerated for the exact proof identity and registry. Cleanup polls until every exact assignment is no longer observable, then deletes and verifies absence of the external deployment record. It waits up to 20 minutes for resource-group deletion because Container Apps managed environments can remain `ScheduledForDelete` well beyond five minutes, uses the authoritative deleted-vault listing to purge the exact proof vault, and fails closed when absence cannot be verified. It refuses to adopt or delete an unrelated group. A timeout reports incomplete and leaves the remaining exact targets available for inspection; it never broadens cleanup to unrelated resources.
