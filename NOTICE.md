# Elsa Control notice

Elsa Control is proprietary commercial deployment-governance and operations
software for professional teams using Elsa Workflows. It is developed and
distributed by Valence Works.

Elsa Workflows is a separate, MIT-licensed, vendor-neutral open-source project.
Elsa Control is optional commercial software and is not Elsa Workflows.
Nothing in the Elsa Control Commercial License claims ownership of Elsa
Workflows or other third-party components.

Third-party and open-source components remain governed by their respective
licences. Their copyright and licence notices must be preserved and distributed
as required by those licences. The generated
[third-party dependency and attribution inventory](THIRD-PARTY-INVENTORY.md)
records the currently resolved package, container-base, and build-action inputs.

Skywalker Digital B.V., trading as Valence Works, is the legal licensor for
Elsa Control. The licence and this notice must be reviewed by qualified legal
counsel before commercial distribution.
