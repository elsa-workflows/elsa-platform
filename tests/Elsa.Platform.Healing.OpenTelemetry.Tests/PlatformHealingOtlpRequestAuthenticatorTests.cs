...
namespace Elsa.Platform.Healing.OpenTelemetry.Tests;

public sealed class PlatformHealingOtlpRequestAuthenticatorTests
{
    [Fact]
    public async Task Active_token_establishes_only_the_server_owned_scope_and_stable_source_identity()
    {
        ...
        result.Context.SourceIdentity.Should().Be($"platform-otel-source:{fixture.Source.Id:D}");
        ...
        result.Context.Metadata["elsa.platform.telemetry-source.credential-version"].Should().Be("1");
    }
    ...
    public async Task Rotation_immediately_invalidates_the_previous_token_and_preserves_source_scope()
    {
        ...
        rotatedResult.Context.SourceIdentity.Should().Be($"platform-otel-source:{fixture.Source.Id:D}");
        rotatedResult.Context.Metadata["elsa.platform.telemetry-source.credential-version"].Should().Be("2");
    ...
    private static async Task<Elsa.Diagnostics.OpenTelemetry.Ingestion.OtlpRequestAuthenticationResult> AuthenticateAsync(
        PlatformHealingOtlpRequestAuthenticator authenticator,
        string token)
    {
    ...
    private static Fixture CreateActiveSource()
    {
        ...
        return new(source, credential, tokens, new PlatformHealingOtlpRequestAuthenticator(store, tokens));
    }
