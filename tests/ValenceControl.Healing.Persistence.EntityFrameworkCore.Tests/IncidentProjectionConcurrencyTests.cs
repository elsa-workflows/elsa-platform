...
using ComponentManifestEntryModel = ValenceControl.Healing.Core.ComponentManifestEntry;
...
public sealed partial class IncidentProjectionConcurrencyTests
{
    ...
    {
        var databasePath = Path.Combine(Path.GetTempPath(), $"valence-control-healing-admission-{Guid.NewGuid():N}.db");
    ...
    {
        var databasePath = Path.Combine(Path.GetTempPath(), $"valence-control-healing-incidents-{Guid.NewGuid():N}.db");
    ...
    {
        var databasePath = Path.Combine(Path.GetTempPath(), $"valence-control-healing-promotion-{Guid.NewGuid():N}.db");
        var options = Options(databasePath);
        var request = Request(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), 0) with
        {
            OccurrenceThreshold = 1,
        ...
        Guid applicationId)
    {
        ...
        db.ComponentManifests.Add(new ComponentManifestModel
        {
            ...
            VerificationMethod = "control-managed-build-attestation",
            CreatedAt = now,
            Entries =
            [
