...
using Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild;
using FluentAssertions;
using Microsoft.Build.Framework;

namespace Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild.Tests;
...
public sealed class GenerateHealingComponentManifestTaskTests
{
    ...
    public void Package_project_declares_publishable_build_assets_and_task_runtime()
    {
        ...
        var projectPath = Path.Combine(projectRoot, "src", "Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild", "Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild.csproj");
        var targetPath = Path.Combine(projectRoot, "src", "Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild", "build", "Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild.targets");
        ...
        targets.Should().Contain("Delete Files=\"$(ElsaHealingComponentManifestOutputPath)\"");
    ...
    public void Packed_package_consumer_clean_removes_a_manifest_generated_by_an_earlier_build()
    {
        ...
        try
        {
            var generatorProject = Path.Combine(repositoryRoot, "src", "Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild", "Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild.csproj");
            ...
            <Project Sdk="Microsoft.NET.Sdk">
              ...
              <ItemGroup>
                <PackageReference Include="Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild" Version="0.0.1" PrivateAssets="all" />
              </ItemGroup>
            </Project>
            """);
            ...
            RunDotNet(consumer, "build", "Consumer.csproj", "-c", "Release", "--no-restore",
                "-p:ElsaHealingSourceRevision=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "-p:ElsaHealingManifestCreatedAt=2026-07-16T00:00:00Z");
            var manifest = Path.Combine(consumer, "bin", "Release", "net10.0", "elsa-healing-component-manifest.json");
    ...
    private sealed class AssetsFixture : IDisposable
    {
            ...
            string? repositoryCommit = null)
        {
            ...
            var output = Path.Combine(project, "obj", "elsa-healing-component-manifest.json");
