...
public sealed class HealingEndToEndTests
{
    ...
    public async Task Otlp_occurrences_group_then_fake_provider_pr_merge_and_positive_deployment_verification_close_the_incident()
    {
        ...
        var appender = new ControlHealingSignalInboxAppender(store);
        ...
        var mergeBody = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(new
        {
            ...
            pull_request = new
            {
                ...
                head = new { @ref = $"valence-control-healing/{attempt.Id:N}", sha = HeadRevision },
                @base = new { sha = BaseRevision }
            }
        }));
        var mergeOutcome = await new ControlHealingGitHubWebhookProcessor(db, new GitHubWebhookProcessor(), time)
        ...
        await deployment.AppendAsync(new DeploymentObservationRequest(
            ...
            MergedRevision, time.GetUtcNow(), DeploymentObservationSources.ControlDeployment,
            "deployment-1", "control-deployment:test", Sha('6'), "deployment:lifecycle"));
    ...
    private static async Task SeedAsync(HealingDbContext db)
    {
        ...
        var manifest = new ComponentManifest
        {
            ...
            VerificationMethod = "control-managed-build-attestation", CreatedAt = Now.AddHours(-1)
        };
        manifest.Entries.Add(new ValenceControl.Healing.Core.ComponentManifestEntry
        {
