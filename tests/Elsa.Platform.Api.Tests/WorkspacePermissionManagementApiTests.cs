...
public sealed class WorkspacePermissionManagementApiTests
{
    ...
    public async Task Owner_manages_contributed_permissions_with_idempotent_audited_mutations()
    {
        ...
        var granted = await owner.PostPlatformJsonAsync(grantsUri, grantRequest);
        var replayedGrant = await owner.PostPlatformJsonAsync(grantsUri, grantRequest);
        ...
        var revoked = await owner.PostPlatformJsonAsync(revocationsUri, revokeRequest);
        var replayedRevoke = await owner.PostPlatformJsonAsync(revocationsUri, revokeRequest);
        ...
        var outsiderContext = await outsider.GetPlatformJsonAsync<MeWorkspacesResponse>("/api/me/workspaces");
        var outsiderGrant = await owner.PostPlatformJsonAsync(
            grantsUri,
    ...
    public async Task Owner_authorization_reads_do_not_restore_a_revoked_default_permission()
    {
        ...
        var revoked = await owner.PostPlatformJsonAsync(
