...
public sealed class WorkspaceRuntimeConfigurationApiTests
{
    ...
    public async Task Workspace_member_can_create_list_and_get_configuration()
    {
        ...
        var create = await client.PostPlatformJsonAsync($"/api/workspaces/{workspaceId}/runtime-configurations", Request("Production"));
        var created = await create.Content.ReadPlatformJsonAsync<WorkspaceRuntimeConfigurationResponse>();
        var list = await client.GetPlatformJsonAsync<IReadOnlyList<WorkspaceRuntimeConfigurationResponse>>($"/api/workspaces/{workspaceId}/runtime-configurations");
        var fetched = await client.GetPlatformJsonAsync<WorkspaceRuntimeConfigurationResponse>($"/api/workspaces/{workspaceId}/runtime-configurations/{created!.Id}");
    ...
    public async Task Workspace_member_can_update_delete_and_clone_configuration()
    {
        ...
        var update = await client.PutPlatformJsonAsync($"/api/workspaces/{workspaceId}/runtime-configurations/{created.Id}", Request("Staging"));
        var updated = await update.Content.ReadPlatformJsonAsync<WorkspaceRuntimeConfigurationResponse>();
        var cloneResponse = await client.PostAsync($"/api/workspaces/{workspaceId}/runtime-configurations/{created.Id}/clone", null);
        var clone = await cloneResponse.Content.ReadPlatformJsonAsync<WorkspaceRuntimeConfigurationResponse>();
    ...
    public async Task Workspace_member_can_create_versions_and_generate_bundle_from_configuration()
    {
        ...
        var version = await versionResponse.Content.ReadPlatformJsonAsync<WorkspaceRuntimeConfigurationVersionResponse>();
        var versions = await client.GetPlatformJsonAsync<IReadOnlyList<WorkspaceRuntimeConfigurationVersionResponse>>($"/api/workspaces/{workspaceId}/runtime-configurations/{created.Id}/versions");
        var bundleResponse = await client.PostAsync($"/api/workspaces/{workspaceId}/runtime-configurations/{created.Id}/bundle", null);
        var bundle = await bundleResponse.Content.ReadPlatformJsonAsync<BuilderBundleResponse>();
    ...
    {
        var response = await client.PostPlatformJsonAsync($"/api/workspaces/{workspaceId}/runtime-configurations", Request(name));
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        return (await response.Content.ReadPlatformJsonAsync<WorkspaceRuntimeConfigurationResponse>())!;
    }
