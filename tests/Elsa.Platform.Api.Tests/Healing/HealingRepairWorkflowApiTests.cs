...
public sealed class HealingRepairWorkflowApiTests
{
    ...
    public async Task Oidc_exchange_returns_only_an_incident_scoped_capability()
    {
        ...
        var grant = await response.Content.ReadPlatformJsonAsync<HealingWorkloadCapabilityResponse>();
    ...
    public async Task Proposal_creation_and_finalization_exchange_use_disjoint_capabilities()
    {
        ...
        var proposal = await createResponse.Content.ReadPlatformJsonAsync<WorkloadProposalCreateResponse>();
        proposal!.Proposal.ProposalId.Should().Be(workload.ProposalId);

        var finalizeResponse = await client.PostPlatformJsonAsync(
        ...
        var grant = await finalizeResponse.Content.ReadPlatformJsonAsync<HealingWorkloadCapabilityResponse>();
        grant!.AllowedScopes.Should().BeEquivalentTo(
    ...
    public async Task Invalid_or_replayed_webhook_is_fail_closed_and_idempotent()
    {
        ...
        var receipt = await replay.Content.ReadPlatformJsonAsync<HealingVerifiedWebhookReceipt>();
        receipt!.IsReplay.Should().BeTrue();
    }

    private static PlatformApiTestApplication CreateApplication(
    ...
    private static HttpRequestMessage Authorized(HttpMethod method, string uri, object? body = null)
    {
        ...
        if (body is not null)
            request.Content = JsonContent.Create(body, options: PlatformApiTestApplication.JsonOptions);
        return request;
    }
