...
public sealed class HealingIntakeApiTests
{
    ...
    public async Task Explicit_incident_uses_route_scope_and_is_idempotent()
    {
        ...
        var accepted = await app.Owner.PostPlatformJsonAsync(uri, request);
        var first = await accepted.Content.ReadFromJsonAsync<JsonElement>();
        var replay = await app.Owner.PostPlatformJsonAsync(uri, request);
        ...
        var persisted = JsonSerializer.Deserialize<HealingSignal>(item.RedactedEnvelopeJson, PlatformApiTestApplication.JsonOptions);
    ...
    public async Task Explicit_incident_rejects_conflicting_replay_and_invalid_scope_or_profile()
    {
        ...
        var occurrenceId = $"explicit-{Guid.NewGuid():N}";
        var uri = ApplicationUri(app, "/incidents");
        (await app.Owner.PostPlatformJsonAsync(uri, Signal(app.ApplicationId, app.EnvironmentId, occurrenceId)))
            .StatusCode.Should().Be(HttpStatusCode.Accepted);

        var conflict = await app.Owner.PostPlatformJsonAsync(
            uri,
            Signal(app.ApplicationId, app.EnvironmentId, occurrenceId) with { OperationName = "POST /changed" });
        var unsupported = await app.Owner.PostPlatformJsonAsync(
            uri,
            Signal(app.ApplicationId, app.EnvironmentId, $"unsupported-{Guid.NewGuid():N}") with { ProfileVersion = "2.0" });
        var missingEnvironment = await app.Owner.PostPlatformJsonAsync(
    ...
    public async Task OpenTelemetry_routes_delegate_with_server_authoritative_scope()
    {
        ...
        var logs = await app.Owner.PostPlatformJsonAsync(
            $"{baseUri}/logs/search",
    ...
    public async Task Explicit_write_and_telemetry_read_require_their_healing_permissions()
    {
        ...
        (await reader.PostPlatformJsonAsync(incidentUri, Signal(app.ApplicationId, app.EnvironmentId, "denied")))
            .StatusCode.Should().Be(HttpStatusCode.Forbidden);
        (await reader.PostPlatformJsonAsync(telemetryUri, new OpenTelemetryLogFilter())).StatusCode.Should().Be(HttpStatusCode.Forbidden);

        await app.Factory.GrantWorkspaceDeploymentPermissionAsync(app.WorkspaceId, accountId, HealingPermissions.Read);
        (await reader.PostPlatformJsonAsync(telemetryUri, new OpenTelemetryLogFilter())).StatusCode.Should().Be(HttpStatusCode.OK);
        (await reader.PostPlatformJsonAsync(incidentUri, Signal(app.ApplicationId, app.EnvironmentId, "still-denied")))
            .StatusCode.Should().Be(HttpStatusCode.Forbidden);
    ...
    private sealed class RecordingTelemetryQuery : IWorkspaceHealingOpenTelemetryQuery
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return ValueTask.FromResult(new CollectorConfiguration(
                new CollectorEndpointInfo("http/protobuf", "https://platform.test/otlp/v1", true, null),
