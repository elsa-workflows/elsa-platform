...
public sealed class OrganizationWorkspaceApiTests
{
    ...
    public async Task Owner_can_create_and_list_organization_workspaces()
    {
        ...
        var context = await owner.GetPlatformJsonAsync<MeWorkspacesResponse>("/api/me/workspaces");
        ...
        var workspace = await create.Content.ReadPlatformJsonAsync<WorkspaceContextResponse>();
        workspace!.Kind.Should().Be(WorkspaceKind.Shared);
        workspace.Role.Should().Be(WorkspaceRole.Owner);

        var list = await owner.GetPlatformJsonAsync<OrganizationWorkspacesResponse>(OrganizationWorkspacesPath(context.Organizations.Single().Id));
    ...
    public async Task Duplicate_active_workspace_name_is_rejected()
    {
        ...
        await app.SeedAsync(_ => Task.CompletedTask);
        var owner = app.CreatePlatformIdentityClient(subject: "owner");
        var organizationId = (await owner.GetPlatformJsonAsync<MeWorkspacesResponse>("/api/me/workspaces"))!.Organizations.Single().Id;
        await AddEntitlementAsync(app, organizationId, maxWorkspaces: 3);
        await owner.PostPlatformJsonAsync(OrganizationWorkspacesPath(organizationId), new OrganizationWorkspaceCreateRequest("Automation"));

        var duplicate = await owner.PostPlatformJsonAsync(OrganizationWorkspacesPath(organizationId), new OrganizationWorkspaceCreateRequest("Automation"));
    ...
    public async Task Concurrent_workspace_creates_cannot_exceed_organization_workspace_limit()
    {
        ...
        var responses = await Task.WhenAll(Enumerable.Range(1, 4)
            .Select(index => owner.PostPlatformJsonAsync(
    ...
    public async Task Concurrent_workspace_creates_cannot_create_duplicate_active_names()
    {
        ...
        var responses = await Task.WhenAll(Enumerable.Range(0, 4)
            .Select(_ => owner.PostPlatformJsonAsync(
                OrganizationWorkspacesPath(organizationId),
                new OrganizationWorkspaceCreateRequest("Automation"))));
    ...
    public async Task Owner_can_rename_and_archive_workspace()
    {
        ...
        var rename = await PatchPlatformJsonAsync(
            owner,
        ...
        var archive = await PatchPlatformJsonAsync(
            owner,
            $"{OrganizationWorkspacesPath(organizationId)}/{workspace.Id}",
            new OrganizationWorkspaceUpdateRequest("Operations", WorkspaceLifecycleStatus.Archived));
        var list = await owner.GetPlatformJsonAsync<OrganizationWorkspacesResponse>(OrganizationWorkspacesPath(organizationId));
    ...
    public async Task Organization_member_lists_only_assigned_workspaces()
    {
        ...
        await app.SeedAsync(_ => Task.CompletedTask);
        var owner = app.CreatePlatformIdentityClient(subject: "owner");
        var ownerContext = await owner.GetPlatformJsonAsync<MeWorkspacesResponse>("/api/me/workspaces");
        ...
        var list = await member.GetPlatformJsonAsync<OrganizationWorkspacesResponse>(OrganizationWorkspacesPath(organizationId));
    ...
    public async Task Other_organization_owner_cannot_list_workspaces()
    {
        ...
        await app.SeedAsync(_ => Task.CompletedTask);
        var owner = app.CreatePlatformIdentityClient(subject: "owner");
        var other = app.CreatePlatformIdentityClient(subject: "other");
        var organizationId = (await owner.GetPlatformJsonAsync<MeWorkspacesResponse>("/api/me/workspaces"))!.Organizations.Single().Id;
        await other.GetPlatformJsonAsync<MeWorkspacesResponse>("/api/me/workspaces");
    ...
    public async Task Workspace_owner_can_be_removed_when_another_owner_exists()
    {
        ...
        await owner.PutPlatformJsonAsync(
    ...
    {
        var response = await owner.PostPlatformJsonAsync(OrganizationWorkspacesPath(organizationId), new OrganizationWorkspaceCreateRequest(name));
        response.StatusCode.Should().Be(HttpStatusCode.Created);
        return (await response.Content.ReadPlatformJsonAsync<WorkspaceContextResponse>())!;
    }

    private static Task<HttpResponseMessage> PatchPlatformJsonAsync<T>(HttpClient client, string requestUri, T value) =>
        client.SendAsync(new HttpRequestMessage(HttpMethod.Patch, requestUri)
        {
            Content = JsonContent.Create(value, options: PlatformApiTestApplication.JsonOptions)
        });

    private static async Task AddEntitlementAsync(PlatformApiTestApplication app, Guid organizationId, int maxWorkspaces)
    {
        await using var scope = app.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<CatalogDbContext>();
    ...
    private static async Task<Guid> AddOrganizationMemberAsync(PlatformApiTestApplication app, Guid organizationId, string subject, OrganizationRole role)
    {
        await using var scope = app.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<CatalogDbContext>();
    ...
    private static async Task<Guid> AddAccountAsync(PlatformApiTestApplication app, string subject)
    {
        await using var scope = app.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<CatalogDbContext>();
        var account = await CreateAccountAsync(db, subject);
