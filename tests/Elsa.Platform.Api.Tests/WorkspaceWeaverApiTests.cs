...
public sealed class WorkspaceWeaverApiTests
{
    ...
    public async Task Configuration_reports_disabled_by_default()
    {
        ...
        var configuration = await response.Content.ReadPlatformJsonAsync<WorkspaceWeaverConfigurationResponse>();
    ...
    {
        await using var app = new PlatformApiTestApplication(FakeWeaverConfiguration());
        await app.SeedAsync(_ => Task.CompletedTask);
        var client = app.CreateTrustedWorkspaceClient("weaver-member");
        var workspaceId = await client.GetDefaultWorkspaceIdAsync();

        var createResponse = await client.PostPlatformJsonAsync(
        ...
        var session = await createResponse.Content.ReadPlatformJsonAsync<WorkspaceWeaverSessionResponse>();
        var messageResponse = await client.PostPlatformJsonAsync(
        ...
        var message = await messageResponse.Content.ReadPlatformJsonAsync<WorkspaceWeaverSendMessageResponse>();
        var detail = await client.GetPlatformJsonAsync<WorkspaceWeaverSessionDetailResponse>(
    ...
    public async Task Plan_mode_prompt_creates_readable_draft_plan()
    {
        ...
        var planDetail = await client.GetPlatformJsonAsync<WorkspaceWeaverPlanResponse>(
    ...
    public async Task Owner_can_approve_and_execute_plan_idempotently()
    {
        ...
        var createResponse = await client.PostPlatformJsonAsync(
            ...
            new WorkspaceWeaverCreateSessionRequest("/admin/deployments", WeaverMode.Plan, new Dictionary<string, string>()));
        var session = await createResponse.Content.ReadPlatformJsonAsync<WorkspaceWeaverSessionResponse>();
        await client.PostPlatformJsonAsync(
            $"/api/workspaces/{workspaceId}/weaver/sessions/{session!.Id}/messages",
            new WorkspaceWeaverSendMessageRequest("Prepare a promotion plan for Production.", WeaverMode.Plan, "Immediate"));
        var detail = await client.GetPlatformJsonAsync<WorkspaceWeaverSessionDetailResponse>(
            $"/api/workspaces/{workspaceId}/weaver/sessions/{session.Id}");
        ...
        var approvalResponse = await client.PostPlatformJsonAsync(
        ...
        var approval = await approvalResponse.Content.ReadPlatformJsonAsync<WorkspaceWeaverPlanApprovalResponse>();
        var firstExecuteResponse = await client.PostPlatformJsonAsync(
            $"/api/workspaces/{workspaceId}/weaver/plans/{plan.Id}/execute",
            new WorkspaceWeaverPlanExecuteRequest(plan.Version));
        var firstExecution = await firstExecuteResponse.Content.ReadPlatformJsonAsync<WorkspaceWeaverPlanExecuteResponse>();
        var secondExecuteResponse = await client.PostPlatformJsonAsync(
            $"/api/workspaces/{workspaceId}/weaver/plans/{plan.Id}/execute",
            new WorkspaceWeaverPlanExecuteRequest(plan.Version));
        var secondExecution = await secondExecuteResponse.Content.ReadPlatformJsonAsync<WorkspaceWeaverPlanExecuteResponse>();
