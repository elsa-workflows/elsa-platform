...
public sealed class BuilderPlanApiTests
{
    ...
    public async Task Public_plan_returns_resolved_state_and_auto_added_shape()
    {
        ...
        var response = await app.CreateClient().PostPlatformJsonAsync("/api/builder/plan", new BuilderPlanApiRequest(MinimalIntent()));

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadPlatformJsonAsync<BuilderPlanApiResponse>();
    ...
    public async Task Workspace_plan_requires_membership()
    {
        ...
        var success = await member.PostPlatformJsonAsync($"/api/workspaces/{workspaceId}/builder/plan", new BuilderPlanApiRequest(MinimalIntent()));
        var anonymous = await app.CreateClient().PostPlatformJsonAsync($"/api/workspaces/{workspaceId}/builder/plan", new BuilderPlanApiRequest(MinimalIntent()));
        var nonMember = await WorkspaceClient(app, "other").PostPlatformJsonAsync($"/api/workspaces/{workspaceId}/builder/plan", new BuilderPlanApiRequest(MinimalIntent()));
