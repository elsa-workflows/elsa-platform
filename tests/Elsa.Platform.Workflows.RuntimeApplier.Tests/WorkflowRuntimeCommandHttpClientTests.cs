...
public sealed class WorkflowRuntimeCommandHttpClientTests
{
    ...
    public async Task Polls_runtime_command_endpoint_and_maps_commands()
    {
        ...
        handler.RequestUri.Should().Be(new Uri($"https://platform.example.test/api/workspaces/{WorkspaceId:D}/deployments/runtime/engines/{EngineId:D}/commands?limit=5"));
    ...
    public async Task Maps_malformed_success_response_to_retryable_result()
    {
        ...
        result.Message.Should().Be("Platform claim response could not be read.");
    }
    ...
    [Fact]
    public async Task Sanitizes_diagnostics_returned_by_platform()
    {
