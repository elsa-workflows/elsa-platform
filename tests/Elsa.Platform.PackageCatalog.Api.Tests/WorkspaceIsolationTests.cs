...
public sealed class WorkspaceIsolationTests
{
    ...
    public async Task Workspace_sources_are_visible_only_to_members()
    {
        ...
        var ownerSources = await context.Owner.GetCatalogJsonAsync<IReadOnlyList<WorkspaceSourceResponse>>($"/api/workspaces/{context.WorkspaceId}/sources");
    ...
    public async Task Workspace_packages_are_hidden_from_anonymous_and_non_members()
    {
        ...
        var ownerPackages = await context.Owner.GetCatalogJsonAsync<IReadOnlyList<PublicPackageResponse>>($"/api/workspaces/{context.WorkspaceId}/packages?sourceIds={context.SourceId}");
        var anonymousPackages = await app.CreateClient().GetCatalogJsonAsync<IReadOnlyList<PublicPackageResponse>>($"/api/packages?sourceIds={context.SourceId}");
    ...
    public async Task Workspace_builder_catalog_is_visible_only_to_members()
    {
        ...
        var ownerCatalog = await context.Owner.GetCatalogJsonAsync<BuilderCatalogResponse>($"/api/workspaces/{context.WorkspaceId}/builder/catalog?sourceIds={context.SourceId}");
    ...
    public async Task Workspace_runtime_configurations_are_isolated_by_workspace()
    {
        ...
        var created = await owner.PostCatalogJsonAsync($"/api/workspaces/{ownerWorkspaceId}/runtime-configurations", RuntimeConfigurationRequest());
        var createdConfiguration = await created.Content.ReadCatalogJsonAsync<WorkspaceRuntimeConfigurationResponse>();
        var nonMember = await ProvisionClientAsync(app, "non-member");
        ...
        var ownWorkspaceList = await nonMember.GetCatalogJsonAsync<IReadOnlyList<WorkspaceRuntimeConfigurationResponse>>($"/api/workspaces/{nonMemberWorkspaceId}/runtime-configurations");
    ...
    private static async Task<WorkspacePackageContext> CreateWorkspaceSourceWithPackageAsync(CatalogApiTestApplication app)
    {
        var owner = app.CreatePlatformIdentityClient(subject: "owner");
        ...
        await admin.PutCatalogJsonAsync($"/api/admin/workspaces/{workspaceId}/entitlements", new WorkspaceEntitlementRequest(true, 2, 500, 20, 25, false));
        var created = await owner.PostCatalogJsonAsync($"/api/workspaces/{workspaceId}/sources", SourceRequest());
    ...
    private static async Task<HttpClient> ProvisionClientAsync(CatalogApiTestApplication app, string subject)
    {
        var client = app.CreatePlatformIdentityClient(subject: subject);
        await client.GetCatalogJsonAsync<MeWorkspacesResponse>("/api/me/workspaces");
        return client;
    }
