...
internal static class WorkspaceDeploymentTestFixtures
{
    ...
    public static HttpClient CreateTrustedWorkspaceClient(this ControlApiTestApplication app, string subject = DefaultSubject)
    {
    ...
    {
        var response = await client.GetControlJsonAsync<MeWorkspacesResponse>("/api/me/workspaces", cancellationToken);
        return response!.Workspaces.Single().Id;
    }

    public static async Task<Guid> AddWorkspaceMemberAsync(this ControlApiTestApplication app, Guid workspaceId, string subject, WorkspaceRole role)
    {
    ...
    public static async Task GrantWorkspaceDeploymentPermissionAsync(this ControlApiTestApplication app, Guid workspaceId, Guid accountId, string permission)
    {
        await using var scope = app.Services.CreateAsyncScope();
