...
usage() {
  cat <<'USAGE'
Usage: scripts/deploy-azure-valence-control.sh [options]

Options:
  --environment <name>       Environment name. Default: dev.
  --resource-group <name>    Azure resource group. Default: rg-valence-control-<environment>.
...
RESOURCE_GROUP="${RESOURCE_GROUP:-rg-valence-control-$ENVIRONMENT_NAME}"
...
IMAGE="$ACR_LOGIN_SERVER/valence-control/api:$IMAGE_TAG"

echo "Building $IMAGE."
az acr login --name "$ACR_NAME"
docker build \
  --platform "$DOCKER_PLATFORM" \
  --file src/ValenceControl.Api/Dockerfile \
  --tag "$IMAGE" \
  .
docker push "$IMAGE"
