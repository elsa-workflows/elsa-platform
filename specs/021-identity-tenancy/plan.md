...
**Testing**: `dotnet test`, with focused API and persistence coverage under `tests/ElsaControl.Api.Tests` and related package catalog test projects.
...
- **Bounded Subsystems**: Pass. Identity and workspace tenancy are implemented through Elsa Control API/Core/Persistence boundaries first; Deployment and Runtime Builder consume workspace authorization through API/service contracts rather than catalog persistence internals.
