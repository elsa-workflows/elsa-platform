...
**Decision**: Add a structured deployability service/model in `Elsa.Platform.Deployment.Core.Workspace` and expose it through workspace-scoped API endpoints before queueing a run.
...
- One command per applier. Rejected because applier ownership is runtime implementation detail and should not leak into Platform dispatch.
...
**Rationale**: The runtime may have changed local state before failure. Platform cannot safely infer rollback semantics across artifact types without runtime-specific recovery logic.

**Alternatives Considered**:

- Automatically rollback successful artifact items. Rejected because Platform does not own runtime-local apply state and rollback may not exist.
