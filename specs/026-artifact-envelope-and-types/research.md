...
**Rationale**: This keeps Valence Control producer-neutral while giving runtime integrations enough metadata to determine whether they can apply an artifact. It also avoids a workflow-specific registry model.

**Alternatives Considered**:

- Store raw workflow definitions in Valence Control. Rejected because Valence Control should not become the workflow runtime source database or interpret workflow internals.
...
- Infer type from payload layout. Rejected because Valence Control should not inspect payload internals to decide applicability.
...
**Rationale**: This preserves the source-of-truth boundary: Valence Control owns deployable artifact records and deployment intent, while payload storage can be provided by producer storage, object storage, OCI, or another provider later.
...
- Make Valence Control understand workflow schema internals. Rejected because that belongs in the workflow runtime applier.
