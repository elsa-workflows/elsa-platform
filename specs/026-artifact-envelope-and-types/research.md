...
**Rationale**: This keeps Platform producer-neutral while giving runtime integrations enough metadata to determine whether they can apply an artifact. It also avoids a workflow-specific registry model.

**Alternatives Considered**:

- Store raw workflow definitions in Platform. Rejected because Platform should not become the workflow runtime source database or interpret workflow internals.
...
- Infer type from payload layout. Rejected because Platform should not inspect payload internals to decide applicability.
...
**Rationale**: This preserves the source-of-truth boundary: Platform owns deployable artifact records and deployment intent, while payload storage can be provided by producer storage, object storage, OCI, or another provider later.
...
- Make Platform understand workflow schema internals. Rejected because that belongs in the workflow runtime applier.
