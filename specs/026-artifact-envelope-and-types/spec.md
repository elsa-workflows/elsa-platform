...
Elsa Control operators and integration authors rely on a stable artifact type catalog so producers and runtime appliers can agree on artifact meaning without the platform needing to understand every payload.

**Why this priority**: Artifact type IDs are the contract between producers, Elsa Control, and runtime integrations. They must be stable, discoverable, and constrained before runtime command sync is introduced.
