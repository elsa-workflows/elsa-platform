...
- [X] T002 [P] Review existing deployment setup form code in `src/Elsa.Platform.Console/src/features/deployments/DeploymentSetupPanel.tsx`
- [X] T003 [P] Review existing deployment workspace persistence code in `src/Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore/DeploymentWorkspaceStore.cs`
...
- [X] T004 Add secret-store and credential-reference models and requests in `src/Elsa.Platform.Deployment.Core/Workspace/WorkspaceDeploymentModels.cs`
- [X] T005 Add secret-store and credential-reference store/service operations in `src/Elsa.Platform.Deployment.Core/Workspace/IWorkspaceDeploymentStore.cs` and `src/Elsa.Platform.Deployment.Core/Workspace/WorkspaceDeploymentService.cs`
- [X] T006 Add secret-store and credential-reference EF entities/configuration in `src/Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore/Models/DeploymentWorkspaceEntities.cs` and `src/Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore/Models/CatalogModelConfiguration.cs`
...
- [X] T008 Implement secret-store and credential-reference persistence in `src/Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore/DeploymentWorkspaceStore.cs`
- [X] T009 Add API contracts and endpoints for secret stores and credential references in `src/Elsa.Platform.Api/Workspace/WorkspaceDeploymentContracts.cs` and `src/Elsa.Platform.Api/Workspace/WorkspaceDeploymentEndpoints.cs`
...
- [X] T011 [P] [US1] Add API test proving environment creation does not require engine data in `tests/Elsa.Platform.Api.Tests/WorkspaceDeploymentApiTests.cs`
- [X] T012 [P] [US1] Add console test proving Add environment form only renders environment fields in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.test.tsx`

### Implementation for User Story 1

- [X] T013 [US1] Replace Add environment usage of combined setup panel with environment-only form in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.tsx`
- [X] T014 [US1] Update Add environment success navigation and empty-state copy in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.tsx`
...
- [X] T015 [P] [US2] Add API test for engine registration with selected credential reference metadata in `tests/Elsa.Platform.Api.Tests/WorkspaceDeploymentApiTests.cs`
- [X] T016 [P] [US2] Add console test for no-engine empty state and Register engine action in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.test.tsx`

### Implementation for User Story 2

- [X] T017 [US2] Update engine registration panel labels and validation in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.tsx`
...
- [X] T019 [P] [US3] Add API tests for creating/listing/archiving secret stores and credential references in `tests/Elsa.Platform.Api.Tests/WorkspaceDeploymentApiTests.cs`
- [X] T020 [P] [US3] Add persistence tests for active/archived secret store and credential reference behavior in `tests/Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore.Tests/DeploymentWorkspacePersistenceTests.cs`
- [X] T021 [P] [US3] Add console tests for secret-store and credential-reference picker behavior in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.test.tsx`

### Implementation for User Story 3

- [X] T022 [US3] Add deployment setup option loading to `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.tsx`
...
- [X] T025 [P] [US4] Add console tests for secret store/reference management interactions in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.test.tsx`

### Implementation for User Story 4

- [X] T026 [US4] Add a Secret stores setup panel to deployment application or environment setup surfaces in `src/Elsa.Platform.Console/src/features/deployments/DeploymentsPage.tsx`
...
- [X] T028 Run API tests with `dotnet test tests/Elsa.Platform.Api.Tests/Elsa.Platform.Api.Tests.csproj --no-restore`
- [X] T029 Run persistence tests with `dotnet test tests/Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore.Tests/Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore.Tests.csproj --no-restore --filter DeploymentWorkspace`
- [X] T030 Run console typecheck with `npm run typecheck` from `src/Elsa.Platform.Console`
- [X] T031 Run console tests with `npm test -- DeploymentsPage` from `src/Elsa.Platform.Console`
