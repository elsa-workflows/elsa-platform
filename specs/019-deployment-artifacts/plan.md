...
- **Contract Stability**: Pass. Layout version is explicit: `platform.elsa.io/deployment-artifact/v1alpha1`.
...
**Structure Decision**: Keep artifact IO in `Elsa.Platform.Deployment.Artifacts`. Manifest parsing/normalization remains in `Elsa.Platform.Deployment.Manifest`; reconciliation remains in `Elsa.Platform.Deployment.Engine`.
...
- Source/test projects exist, are added to `Elsa.Platform.sln`, and reference only allowed packages.
