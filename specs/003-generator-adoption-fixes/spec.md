...
1. **Given** a multi-targeted module project references `Elsa.Specifications.PackageManifest.Generator` with private assets, **When** the maintainer runs a normal build, **Then** the build succeeds and creates a manifest for the discovered shell features.
