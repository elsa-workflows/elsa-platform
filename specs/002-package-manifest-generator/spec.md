...
**Input**: User description: "Create a specification for a build-time package called Elsa.Specifications.PackageManifest.Generator."
...
The generated manifest is the distribution contract consumed later by Elsa Control Package Catalog, Elsa Control Runtime Builder, professional Elsa Docker images, and future runtime validation tools. The generator must emit manifests using the shared `Elsa.Specifications.PackageManifests` wire contract package and must not define or serialize a separate manifest model.
...
- Reuse `Elsa.Specifications.PackageManifests` as the canonical manifest contract and validation source.
...
- Validate the final manifest against the versioned schema from `Elsa.Specifications.PackageManifests`.
...
- Implementing Elsa Control Package Catalog ingestion or APIs.
...
- Replacing `Elsa.Specifications.PackageManifests` with generator-owned DTOs.
...
- **Manifest Contract Maintainer**: Evolves `Elsa.Specifications.PackageManifests` and needs the generator to follow the shared wire contract without drift.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Generate Manifest Automatically (Priority: P1)

A package author adds a private build-time reference to `Elsa.Specifications.PackageManifest.Generator` and receives a generated `elsa-package.json` during build or pack.
...
- Validation schema version from `Elsa.Specifications.PackageManifests` is unsupported or unavailable.
...
- **FR-001**: The generator MUST be consumable by class library projects through a package reference named `Elsa.Specifications.PackageManifest.Generator` with `PrivateAssets="all"`.
...
- **FR-008**: The generator MUST emit manifests using the `Elsa.Specifications.PackageManifests` contract and MUST NOT define an independent wire model for generated JSON.
- **FR-009**: The generator MUST validate the final manifest against the versioned schema supplied by `Elsa.Specifications.PackageManifests`.
...
- **FR-071a**: Optional generator-owned hint attributes SHOULD be provided as source-only compile assets from `Elsa.Specifications.PackageManifest.Generator` so consuming packages can use hints without emitting a runtime dependency for them.
...
- **FR-071c**: Source-only manifest hint attributes SHOULD use the namespace `Elsa.Specifications.PackageManifest.Generator.Hints`.
...
The default integration is an MSBuild task distributed by `Elsa.Specifications.PackageManifest.Generator`. A separate `Elsa.Specifications.PackageManifest.Generator.MSBuild` package may be used internally if it simplifies packaging, but package authors should reference only `Elsa.Specifications.PackageManifest.Generator` for the standard experience.
...
- `Elsa.Specifications.PackageManifest.Generator`: Public package referenced by package authors; brings in build assets, task assets, and optional source-only compile assets for manifest hints.
- `Elsa.Specifications.PackageManifest.Generator.Core`: Optional internal/shared library for generation logic if it removes meaningful duplication between task, tests, or future analyzers.
- `Elsa.Specifications.PackageManifest.Generator.MSBuild`: Optional packaging/task assembly if separating MSBuild assets keeps the public package cleaner.
- `Elsa.Specifications.PackageManifests`: Required shared contract package for manifest DTOs, schema resources, validation behavior, and serialization rules.
...
11. Build the final `Elsa.Specifications.PackageManifests` contract object.
...
CShells metadata is the primary annotation source for feature discovery and feature metadata. Optional manifest hints enrich inference and reduce override-file noise, but the generated JSON still uses `Elsa.Specifications.PackageManifests`.
...
Rich metadata such as long documentation, complex compatibility matrices, icon metadata, feature conflicts, required capabilities, and broad extension payloads should live in the override file unless the `Elsa.Specifications.PackageManifests` contract later defines a stronger hint need.
...
For each feature setting, the generator produces JSON Schema metadata consistent with `Elsa.Specifications.PackageManifests`.
...
- **SC-005**: Generated manifests validate against the active `Elsa.Specifications.PackageManifests` schema in all valid sample projects.
...
- `Elsa.Specifications.PackageManifests` exists or will be created as the shared manifest contract package with versioned JSON Schema resources and validation behavior.
...
1. A library project can reference `Elsa.Specifications.PackageManifest.Generator` and get an `elsa-package.json` generated automatically.
2. The generated manifest uses the `Elsa.Specifications.PackageManifests` contract.
