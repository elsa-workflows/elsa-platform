...
hints in `ValenceControl.PackageManifest.Generator.Hints` can cover these narrow cases
without adding runtime dependencies or replacing `ValenceControl.PackageManifests`.

Alternatives considered:

- Put hints in `ValenceControl.PackageManifests`: blurs generator input metadata with
  manifest DTOs.
