...
hints in `Elsa.Specifications.PackageManifest.Generator.Hints` can cover these narrow cases
without adding runtime dependencies or replacing `Elsa.Specifications.PackageManifests`.

Alternatives considered:

- Put hints in `Elsa.Specifications.PackageManifests`: blurs generator input metadata with
  manifest DTOs.
