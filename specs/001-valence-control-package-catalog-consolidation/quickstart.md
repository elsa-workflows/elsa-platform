# Quickstart: Valence Control Package Catalog Consolidation
...
find specs/001-valence-control-package-catalog-consolidation -maxdepth 3 -type f | sort
```
...
- Valence Control import verification: `dotnet test ValenceControl.sln` passed from `valence-control`, including PR #36 builder/runtime tests. The same NU1903 advisory warnings were present.
- Valence Control console verification: `npm install --prefix src/ValenceControl.Console && npm test --prefix src/ValenceControl.Console -- --run` passed with 11 files and 64 tests. The same npm audit and React Router warnings were present.
- Merge conflicts were limited to expected repository bootstrap files. `valence-control` Spec Kit state, `AGENTS.md`, `README.md`, and `LICENSE` were kept as authoritative.
...
Before committing the merge, resolve path conflicts by keeping `valence-control` Spec Kit infrastructure active and moving catalog files into their target platform locations. If a full unrelated-history merge proves too noisy, use a subtree-style import and document the fallback:
...
- `.specify/`
- `AGENTS.md`
- `docs/valence-control-deployment-phased-strategy.md`
...
- `Elsa.PackageCatalog.sln` was renamed to `ValenceControl.sln`.
- Source, test, namespace, package, and console package names were normalized to `ValenceControl.PackageCatalog.*`, `ValenceControl.PackageManifests`, and `ValenceControl.PackageManifest.Generator*`.
- `ValenceControl.PackageCatalog.Abstractions` was added with compatibility validation request/result contracts. Core and API reference it directly; entity and persistence contracts remain in Core/Persistence until a later deployment-facing boundary needs them.
- `dotnet test ValenceControl.sln` passed after renames and abstraction extraction. The run still reports the existing `Microsoft.Build.Utilities.Core` NU1903 advisory warnings.
- `npm install --prefix src/ValenceControl.Console` reported 5 moderate npm audit findings.
- `npm test --prefix src/ValenceControl.Console -- --run` passed with 11 files and 64 tests. React Router v7 future-flag warnings remain pre-existing.
...
- Added `ValenceControl.RuntimeBuilder.Abstractions`, `ValenceControl.RuntimeBuilder.Core`, and `ValenceControl.RuntimeBuilder.DeploymentTemplates`.
...
- Moved Runtime Builder core services and file renderers into `ValenceControl.RuntimeBuilder.Core`.
- Moved deployment template renderers into `ValenceControl.RuntimeBuilder.DeploymentTemplates`.
- Added catalog-facing query and compatibility contracts to `ValenceControl.PackageCatalog.Abstractions` so Runtime Builder Core references catalog abstractions, not catalog core, API, or EF persistence.
- Kept the current HTTP endpoints hosted by `ValenceControl.Api` for now; extracting `ValenceControl.RuntimeBuilder.Api` remains a later API packaging step.
...
- `dotnet build ValenceControl.sln` passed. The run still reports the existing `Microsoft.Build.Utilities.Core` NU1903 advisory warnings.
- `dotnet test ValenceControl.sln` passed. Runtime Builder tests now run under `tests/ValenceControl.RuntimeBuilder.Core.Tests` with 27 tests.
- `npm test --prefix src/ValenceControl.Console -- --run` passed with 11 files and 64 tests. React Router v7 future-flag warnings remain pre-existing.
...
```bash
npm install --prefix src/ValenceControl.Console
npm test --prefix src/ValenceControl.Console
```
...
- README points to `valence-control`.
...
- `ValenceControl.PackageCatalog.Abstractions` exposes deployment-facing package requirement validation contracts.
...
- Added `IDeploymentPackageCatalog` and package requirement validation DTOs under `ValenceControl.PackageCatalog.Abstractions.Deployment`.
- Added `tests/ValenceControl.PackageCatalog.Abstractions.Tests`.
- Added a boundary test that scans `src/ValenceControl.Deployment.*` project references and fails if future Deployment projects reference catalog internals.
- Added contract tests proving approval, trust, suspicious-change, and compatibility states remain distinct.
- Updated `docs/valence-control-deployment-phased-strategy.md` to state that Deployment consumes deployment-specific manifests/artifacts and catalog validation contracts, not Runtime Builder intent directly.
- `dotnet test ValenceControl.sln` passed. The existing `Microsoft.Build.Utilities.Core` NU1903 advisory warnings remain.

## 8. Final Readiness Validation

Phase final verification log, 2026-05-19:

- `dotnet test ValenceControl.sln` passed.
- `npm test --prefix src/ValenceControl.Console -- --run` passed with 11 files and 64 tests.
