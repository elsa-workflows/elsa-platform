# Feature Specification: Valence Control Package Catalog Consolidation

**Feature Branch**: `001-valence-control-package-catalog-consolidation`

**Created**: 2026-05-19

**Status**: Draft

**Input**: User description: "Reimagine the current `elsa-package-catalog` repository as Valence Control subsystems, move toward the ideal package layout under `valence-control`, deprecate the old repository, improve architecture where useful, incorporate merged PR #36 Runtime Builder backend foundations, and use Spec Kit for implementation."
...
- Q: Which Spec Kit state wins when repositories conflict? -> A: Keep `valence-control` Spec Kit infrastructure active and import catalog specs without overwriting `.specify/feature.json`.
...
### User Story 1 - Establish Valence Control Home (Priority: P1)

Elsa maintainers can see the package catalog, package manifests, and package manifest generator represented as first-class Valence Control subsystems with clear ownership, naming, boundaries, and migration phases.
...
Elsa maintainers can import the current package catalog work into `valence-control`, keep existing behavior testable, and avoid mixing mechanical movement with architectural rewrites.
...
**Independent Test**: Verify the platform solution contains target projects named `ValenceControl.PackageCatalog.*`, `ValenceControl.PackageManifests`, and `ValenceControl.PackageManifest.Generator*`, with dependency direction enforced by project references and tests.

**Acceptance Scenarios**:

1. **Given** catalog code has been imported, **When** package names are normalized, **Then** Package Catalog projects use `ValenceControl.PackageCatalog.*`.
...
1. **Given** imported catalog code contains `Builder`, `DeploymentTemplates`, and `RuntimeConfigurations` areas, **When** package boundaries are normalized, **Then** those areas move toward `ValenceControl.RuntimeBuilder.*` projects.
...
Elsa maintainers and contributors know that active package catalog development has moved to `valence-control`, and the old `elsa-package-catalog` repository remains useful only as historical reference or an archived redirect.
...
- Spec Kit files from the catalog repository conflict with Spec Kit files in `valence-control`.
...
- **FR-002**: The target source layout MUST include `ValenceControl.PackageCatalog.*`, `ValenceControl.PackageManifests`, and `ValenceControl.PackageManifest.Generator*` projects.
...
- **FR-011**: The plan MUST define compatibility handling for existing `ValenceControl.PackageManifests` and `ValenceControl.PackageManifest.Generator` package identities before renaming published package IDs.
...
- **FR-017**: Runtime Builder backend capabilities introduced by `elsa-package-catalog#36` MUST be mapped to a dedicated `ValenceControl.RuntimeBuilder.*` subsystem during architecture normalization.
...
- **FR-020**: Importing catalog Spec Kit files MUST NOT overwrite the active `valence-control` `.specify/feature.json` or platform Spec Kit installation.
...
- **Valence Control Subsystem**: A bounded area within `valence-control`, such as Deployment or Package Catalog, with source projects, tests, specs, and ownership rules.
...
- **Runtime Builder Subsystem**: Valence Control subsystem that owns builder intent, runtime image metadata, server-side planning, generated bundles, deployment templates, and saved runtime configurations.
...
- **Deprecation Notice**: Documentation and repository state that redirects contributors from the old catalog repository to `valence-control`.
...
- **SC-002**: The first implementation phase can be verified by building the imported catalog code and running its existing test suites from `valence-control`.
