# Research: Valence Control Package Catalog Consolidation

## Decision: Consolidate Package Catalog Into Valence Control
...
## Decision: Rename Toward `ValenceControl.*`
...
**Rationale**: NuGet is the first source type, but catalog core should not assume NuGet. Moving NuGet sync into `ValenceControl.PackageCatalog.Sources.NuGet` makes future source types possible without catalog core churn.
...
- Treat these capabilities as `ValenceControl.RuntimeBuilder.*`, not as Package Catalog core.
...
- Deployment may later consume Runtime Builder artifacts or contracts, but live reconciliation remains in `ValenceControl.Deployment.*`.
...
- Map `src/ValenceControl.PackageCatalog.Core/Builder/*` to `ValenceControl.RuntimeBuilder.Core`.
- Map `src/ValenceControl.PackageCatalog.Core/DeploymentTemplates/*` to `ValenceControl.RuntimeBuilder.DeploymentTemplates`.
- Map `src/ValenceControl.PackageCatalog.Core/RuntimeConfigurations/*` and persistence stores to Runtime Builder contracts/persistence unless implementation feedback shows they should remain catalog-owned.
...
- Keep `valence-control` `.specify/` as the active Spec Kit installation.
...
- These package IDs are not published on nuget.org yet, so renaming to `ValenceControl.PackageManifests` and `ValenceControl.PackageManifest.Generator` is likely safe.
...
- The old repository README was updated on `main` in commit `cf7411d` to mark active development as moved to `https://github.com/valence-works/valence-control`.
