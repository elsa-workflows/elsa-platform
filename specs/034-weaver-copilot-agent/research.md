...
**Rationale**: GitHub Copilot SDK is generally available as of 2026-06-02 and provides the agent loop, custom tools, custom agents, streaming, hooks, telemetry, session resume, and BYOK support. The .NET SDK matches Elsa Platform's backend stack and avoids building a custom orchestration layer.
...
**Rationale**: Weaver is a platform operations assistant, not a code editing agent inside the console. Hosted sessions should expose only typed Elsa Platform tools with explicit schemas and authorization.
...
**Rationale**: First-party Elsa Platform data can be exposed more safely through in-process typed tools that use existing services and permission checks. MCP remains useful later for GitHub, documentation, or customer-support systems.
