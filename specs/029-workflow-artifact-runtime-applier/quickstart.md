...
- 2026-05-29: `dotnet test tests/Elsa.Platform.Workflows.RuntimeApplier.Tests/Elsa.Platform.Workflows.RuntimeApplier.Tests.csproj --no-restore -v minimal` passed: 78 tests. This adds HTTP(S) workflow artifact payload loading with provider/host approval, private-address blocking, pinned transport connections, proxy bypass, redirect rejection, expired-reference, URI/scheme, concrete media-type, response content-type fallback, size-limit, remote failure, caller-owned transport disposal, and safe error handling coverage.
