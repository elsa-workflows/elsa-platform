...
**Input**: User description: "Create the Elsa Workflows runtime integration package that consumes Platform deployment commands, validates workflow artifacts, applies supported workflow definition artifacts to the local runtime store, and reports safe progress and outcomes back to Platform."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Runtime Applies Workflow Artifact (Priority: P1)

A runtime operator installs the Elsa Workflows Platform integration so a runtime can poll Platform, claim a deployment command, fetch the artifact payload, validate it, and install the workflow definition into the local runtime store.

**Why this priority**: This is the first artifact consumer and completes the deploy path from Platform command to runtime application.
...
Platform users can see runtime apply progress and final status without Platform understanding workflow internals.

**Why this priority**: The Platform deployment console must remain the operator-facing source of truth while the runtime owns interpretation and apply.

**Independent Test**: Run a deployment that reports validation, apply progress, success, failure, and rejection states, then verify Platform run history shows safe events.
...
2. **Given** apply succeeds, **When** the runtime completes the command, **Then** Platform records observed digest and runtime reference.
3. **Given** apply fails, **When** the runtime reports failure, **Then** Platform stores safe diagnostics and marks the run failed.

---

### User Story 3 - Advertise Runtime Compatibility (Priority: P2)

The runtime integration advertises artifact type support, schema compatibility, and required capabilities so Platform validation can target commands correctly.

**Why this priority**: Platform should route workflow artifacts only to runtimes that can interpret them.

**Independent Test**: Register heartbeat capabilities and verify Platform validation accepts compatible artifacts and blocks incompatible artifact/schema combinations.

**Acceptance Scenarios**:

1. **Given** the runtime integration starts, **When** it sends heartbeat metadata, **Then** Platform records workflow artifact capability support.
...
- Runtime restarts after applying locally but before completing the Platform command.
...
- **FR-002**: Runtime integration MUST poll Platform runtime command APIs and claim commands with a lease.
...
- **FR-007**: Runtime integration MUST post heartbeat, progress, completion, failure, or rejection to Platform using the active lease token.
...
- **Runtime Sync Worker**: Runtime-side background process that polls Platform and owns command claim/heartbeat/completion calls.
...
- **Runtime Apply Result**: Safe status, observed digest, runtime reference, and diagnostics reported to Platform.
...
- **SC-003**: Digest mismatch, unsupported schema, and local validation failure all produce safe Platform-visible rejection or failure states.
- **SC-004**: Runtime progress appears in Platform run history within 10 seconds during the integration test.
...
- Platform runtime command APIs and artifact envelope metadata already exist.
