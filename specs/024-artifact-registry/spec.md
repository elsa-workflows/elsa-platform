...
Elsa Control is the source of truth for deployable workflow artifacts, while Elsa Studio remains the authoring surface and Elsa runtimes remain the execution surface. In Elsa Control-integrated Studio installations, the user-facing handoff command is **Submit to Elsa Control**. That command creates an immutable platform artifact and does not mean release, promotion, deployment, or immediate runtime execution.
...
- Artifact metadata references a layout version other than `elsa-control/deployment-artifact/v1alpha1`.
...
- A Elsa Control-integrated Studio submission uses Publish terminology instead of "Submit to Elsa Control"; the UX must distinguish direct runtime publishing from platform artifact submission.
...
- **FR-015**: System MUST treat future Elsa Studio "Submit to Elsa Control" submissions as artifact ingestion events that produce immutable platform artifact records without implying release approval, promotion, deployment, or immediate runtime execution.
...
- Existing `ElsaControl.Deployment.Artifacts` contracts are the source of truth for artifact layout, metadata, checksum, and inspection concepts where they fit.
- The first Studio integration should use "Submit to Elsa Control" for the artifact handoff; "Publish" remains direct-runtime terminology unless a non-integrated Studio installation uses the existing behavior.
