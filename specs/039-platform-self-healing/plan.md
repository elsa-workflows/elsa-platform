...
- `elsa-platform` owns the Healing signal profile, client and component-manifest packages, all incident/repair/policy state, OpenTelemetry bridge, GitHub and agent adapters, API, persistence, workers, and Console experience.
...
**Target Platform**: Linux-hosted Elsa Platform control plane; .NET and ASP.NET Core monitored applications; GitHub-hosted repositories and GitHub Actions runners
...
**Constraints**: Post-redaction evidence only; workspace isolation; idempotent durable intake; no agent access to Git write credentials; no self-modifying repairs; maximum two automatic attempts by default; Platform observes but never deploys or rolls back
...
|---|---|---|
| I. Control Plane First | Platform owns governance, repair orchestration, provider projections, and verification records. Runtime workflow state, deployment, and rollback remain outside Healing. | PASS |
...
1. **Durable intake before background analysis**: Foundation calls additive contributors after redaction and before telemetry storage/live publication. Platform's contributor performs only an idempotent durable inbox append. Classification, deduplication, attribution, and repair dispatch happen in leased background workers.
2. **Platform incident is canonical**: GitHub issues, labels, comments, workflows, branches, and pull requests are projections or commands. They never become the source of truth.
...
7. **Cross-repository sequencing**: Foundation's generic contribution contract lands first. Platform consumes the published package version and verifies the combined OTLP-to-inbox path with a local-package integration lane before Platform merge.
8. **Independent persistence ownership**: Healing uses a dedicated context, migrations, stores, and migration history. It may share a physical database and workspace/application identifiers with other Platform subsystems without making Package Catalog persistence its owner.
...
```text
specs/039-platform-self-healing/
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
...
```text
src/
├── Elsa.Platform.Healing.Abstractions/
├── Elsa.Platform.Healing.Core/
│   ├── Configuration/
│   ├── Incidents/
│   ├── Manifests/
│   ├── Ownership/
...
│   ├── Security/
│   └── Verification/
├── Elsa.Platform.Healing.Agent/
├── Elsa.Platform.Healing.GitHub/
├── Elsa.Platform.Healing.OpenTelemetry/
├── Elsa.Platform.Healing.Client/
├── Elsa.Platform.Healing.ComponentManifest/
├── Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild/
├── Elsa.Platform.Healing.Persistence.EntityFrameworkCore/
├── Elsa.Platform.Healing.Persistence.SqliteMigrations/
├── Elsa.Platform.Healing.Persistence.SqlServerMigrations/
├── Elsa.Platform.Api/Workspace/Healing/
└── Elsa.Platform.Console/src/features/healing/

tests/
├── Elsa.Platform.Healing.Abstractions.Tests/
├── Elsa.Platform.Healing.Core.Tests/
├── Elsa.Platform.Healing.Agent.Tests/
├── Elsa.Platform.Healing.GitHub.Tests/
├── Elsa.Platform.Healing.OpenTelemetry.Tests/
├── Elsa.Platform.Healing.Client.Tests/
├── Elsa.Platform.Healing.ComponentManifest.Tests/
├── Elsa.Platform.Healing.ComponentManifest.Generator.MSBuild.Tests/
├── Elsa.Platform.Healing.Persistence.EntityFrameworkCore.Tests/
├── Elsa.Platform.Api.Tests/Healing/
└── Elsa.Platform.Console/src/features/healing/*.test.tsx
```
...
**Structure Decision**: Healing receives dependency-light abstractions plus explicit core, persistence, agent, OpenTelemetry bridge, and provider adapters. Existing Platform host projects remain composition roots for API and Console UI. The Foundation change is deliberately a generic telemetry extension point so neither repository reverses its ownership boundary.

## Delivery Gates

1. Foundation contribution ordering/redaction/failure tests pass and the package is available to Platform.
2. Platform contracts, models, and persistence migrations pass SQLite and SQL Server model verification.
...
10. Foundation PR merges and its artifact is consumable before the dependent Platform PR merges.
