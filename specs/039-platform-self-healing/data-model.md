# Data Model: Platform Self-Healing

All authoritative entities are workspace-scoped unless explicitly platform-global. IDs are immutable GUIDs. External provider numeric/string IDs are stored separately from Platform IDs. Every mutable aggregate uses an optimistic concurrency token and normalizes timestamps to UTC.
...
`HumanCommand` stores the normalized retry/stop/evidence/waiver request, provider actor, linked Platform actor, provider permission snapshot, workspace permission decision, confirmation reference, status, and result. Provider text is never executed as instructions.

## 18. DeploymentObservation and VerificationResult

`DeploymentObservation` is unique by source/idempotency key and records workspace/application/environment/revision/deployed time, source (`PlatformDeployment`, `ExternalDelivery`), trust identity, and evidence digest.
...
| `ActorType`, `ActorId` | values | Human, Platform, GitHub, workflow, agent |
