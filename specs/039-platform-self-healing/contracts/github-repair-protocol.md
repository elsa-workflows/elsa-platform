...
Platform creates one issue per active incident episode/repository with:

- Stable machine marker containing Platform incident/episode IDs.
...
- Impact, affected environments/revisions, attribution, evidence tier, attempts, current blockers, repair/PR/verification state, and Platform deep link.
- GitHub-specific labels such as `elsa-healing`, severity, state, and `needs-human` as projections only.

Occurrence updates do not create one comment per signal. Manual edits outside the machine-owned section are preserved.

## Workflow dispatch

Platform dispatches the approved workflow identity at its approved immutable revision with a minimal payload:
...
The workflow requests a GitHub OIDC token with the Platform audience and exchanges it with the one-time nonce. Platform validates the signed claims against the source ownership binding and attempt before returning a capability token.
...
Repository/source/log/test content remains untrusted. Result text is never interpreted as a Platform command.

## Trusted publication

Before provider mutation, Platform independently:
...
Forbidden automated changes always include Platform-supplied Healing workflow files, publisher/permission/validation policy files, CODEOWNERS/branch-protection automation, and paths configured as self-protecting guardrails.
...
Supported normalized commands are `retry`, `stop`, `request-evidence`, and `waive-environment`. A GitHub comment/label is only a request. Platform verifies webhook, repository permission, linked Platform identity, workspace permission, and any required confirmation before recording/executing the command.
