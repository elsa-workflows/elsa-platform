...
**Scope**: Elsa Platform implementation plus the merged Elsa Foundation OpenTelemetry ingestion seam  
...
| Webhook payload identifiers and control-shaped strings were insufficiently constrained. | Positive PR numbers, bounded revisions, event/action allow-lists, HMAC validation, replay binding, and safe normalized commands are enforced. | `GitHubWebhookProcessorTests`, `PlatformHealingVerifiedWebhookHandlerTests`. |
...
- The repair agent never receives a GitHub write credential; Platform's trusted publisher performs mutations.
