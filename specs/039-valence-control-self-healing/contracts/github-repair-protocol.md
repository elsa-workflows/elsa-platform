...
Valence Control creates one issue per active incident episode/repository with:

- Stable machine marker containing Valence Control incident/episode IDs.
...
- Impact, affected environments/revisions, attribution, evidence tier, attempts, current blockers, repair/PR/verification state, and Valence Control deep link.
- GitHub-specific labels such as `valence-control-healing`, severity, state, and `needs-human` as projections only.

Occurrence updates do not create one comment per signal. Manual edits outside the machine-owned section are preserved.

## Workflow dispatch

Valence Control dispatches the approved workflow identity at its approved immutable revision with a minimal payload:

- protocol version
- Valence Control base URL
- incident ID
- episode ID
- attempt ID
- one-time nonce
...
The workflow requests a GitHub OIDC token with the Valence Control audience and exchanges it with the one-time nonce. Valence Control validates the signed claims against the source ownership binding and attempt before returning a capability token.
...
The capability is not an authority snapshot. Exchange, evidence, proposal, heartbeat, finalization exchange, and result upload each revalidate the current Valence Control/workspace/application/environment repair gates, active incident episode, provider connection, and source binding. Revocation immediately invalidates and durably revokes outstanding capability exchanges.

Before managed inference, Valence Control commits an at-most-once leased reservation for the attempt and its full inference-unit allowance. Concurrent callers cannot invoke the provider twice. If Valence Control crashes after invocation but before the atomic proposal/audit commit, an expired reservation is treated as indeterminate and is not reacquired unless a future provider contract supplies durable provider-side idempotency; the attempt is released to an audited `NeedsHuman` outcome instead.
...
Repository/source/log/test content remains untrusted. Result text is never interpreted as a Valence Control command.

## Trusted publication

Before provider mutation, Valence Control independently:
...
Forbidden automated changes always include Valence Control-supplied Healing workflow files, publisher/permission/validation policy files, CODEOWNERS/branch-protection automation, and paths configured as self-protecting guardrails.
...
Valence Control observes PR/check/branch-protection changes through verified webhooks and provider refreshes. Auto-merge is requested only when a fresh complete policy evaluation passes. GitHub branch protection and repository policy remain final authority.

Valence Control records the merged SHA but does not mark the incident healed until deployment verification succeeds.

### Repository validation harness

If `.elsa/healing/validate` exists and is executable, the isolated validation job sets
`VALENCE_CONTROL_HEALING_VALIDATION_OUTPUT` to a runner-owned file. The harness may replace that file with only this
...
claims fail validation. Repository text is never copied from this file into Valence Control or the pull request. Missing
evidence remains false and therefore human-merge-only.

## Human commands

Supported normalized commands are `retry`, `stop`, `request-evidence`, and `waive-environment`. A GitHub comment/label is only a request. Valence Control verifies webhook, repository permission, linked Valence Control identity, workspace permission, and any required confirmation before recording/executing the command.
