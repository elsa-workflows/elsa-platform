# Quickstart: Valence Control Self-Healing
...
- Clean feature branches in both `elsa-foundation` and `valence-control`
...
1. Build the sample ASP.NET Core workflow host with the Valence Control component-manifest package.
...
3. Upload the manifest to one Valence Control application revision.
...
2. Verify Valence Control may create an explicitly unreproduced draft PR when policy permits, but auto-merge is impossible.
...
6. Send the matching exception again; verify failed verification and a trusted failure signal, without Valence Control rollback.
...
## Valence Control validation

```bash
dotnet test tests/ValenceControl.Healing.Abstractions.Tests/ValenceControl.Healing.Abstractions.Tests.csproj
dotnet test tests/ValenceControl.Healing.Core.Tests/ValenceControl.Healing.Core.Tests.csproj
dotnet test tests/ValenceControl.Healing.Persistence.EntityFrameworkCore.Tests/ValenceControl.Healing.Persistence.EntityFrameworkCore.Tests.csproj
dotnet test tests/ValenceControl.Healing.OpenTelemetry.Tests/ValenceControl.Healing.OpenTelemetry.Tests.csproj
dotnet test tests/ValenceControl.Healing.GitHub.Tests/ValenceControl.Healing.GitHub.Tests.csproj
dotnet test tests/ValenceControl.Healing.Agent.Tests/ValenceControl.Healing.Agent.Tests.csproj
dotnet test tests/ValenceControl.Api.Tests/ValenceControl.Api.Tests.csproj --filter Healing
dotnet build ValenceControl.sln
npm --prefix src/ValenceControl.Console test -- src/features/healing
npm --prefix src/ValenceControl.Console run typecheck
npm --prefix src/ValenceControl.Console run build
git diff --check
```
...
3. OIDC exchange with Valence Control audience.
