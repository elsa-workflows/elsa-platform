# Research: Elsa Control Self-Healing
...
**Rationale**: Elsa Control must durably accept exception candidates without putting Healing concepts into Foundation. Awaiting the Elsa Control contributor means an OTLP success response is not returned until its idempotent inbox append succeeds. Worker downtime then cannot lose accepted repair work.
...
**Rationale**: Intake remains fast and retry-safe while every later side effect is resumable. The existing deployment command lease and webhook-dispatch workers provide proven Elsa Control conventions.
...
**Decision**: Create `HealingDbContext`, provider-neutral EF stores, SQLite migrations, and SQL Server migrations under Healing-owned projects. The context may use the same connection string/physical database as other Elsa Control subsystems but keeps a separate migration history.
...
**Decision**: Use standard `exception.type`, `exception.message`, `exception.stacktrace`, `error.type`, service/resource, trace, span, and severity semantics. Add versioned `elsa.control.healing.*` attributes only for product-specific identity, classification, retry exhaustion, operation identity, application/environment/revision, component-manifest reference, occurrence ID, and explicit-report intent.
...
- Requiring the Elsa Control client: rejected because conformance is behavioral; the client is only a reference implementation.
...
**Decision**: A Elsa Control-owned MSBuild package generates a canonical JSON manifest for one application revision from resolved NuGet assets and managed assemblies. It records component/package/assembly identity, versions, dependency edges, content hashes, available source metadata, build/source revision, and a manifest digest. Upload associates that digest with a Elsa Control application revision.
...
**Decision**: Elsa Control validates GitHub's issuer, Elsa Control-specific audience, signature, expiry, JWT ID, immutable repository identity, repository/workflow claims, `job_workflow_ref`, ref, SHA, run ID/attempt, and a one-time incident attempt nonce. It returns a short-lived capability token limited to one attempt's evidence read, heartbeat, and result upload.

**Rationale**: GitHub Actions OIDC provides signed repository/workflow/run claims without storing a long-lived Elsa Control secret in the repository. `id-token: write` grants only token issuance, not repository write access.
...
**Rationale**: GitHub branch protection and required checks remain final provider-side constraints, but Elsa Control must prove its stricter product policy first.
...
**Rationale**: Network retries and multiple Elsa Control replicas otherwise create duplicate issues, branches, PRs, or merge requests. Existing deployment leases are the closest reusable pattern.
