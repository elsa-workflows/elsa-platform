# Quickstart: Elsa Control Self-Healing
...
- Clean feature branches in both `elsa-foundation` and `elsa-control`
...
1. Build the sample ASP.NET Core workflow host with the Elsa Control component-manifest package.
...
3. Upload the manifest to one Elsa Control application revision.
...
2. Verify Elsa Control may create an explicitly unreproduced draft PR when policy permits, but auto-merge is impossible.
...
6. Send the matching exception again; verify failed verification and a trusted failure signal, without Elsa Control rollback.
...
## Elsa Control validation

```bash
dotnet test tests/ElsaControl.Healing.Abstractions.Tests/ElsaControl.Healing.Abstractions.Tests.csproj
dotnet test tests/ElsaControl.Healing.Core.Tests/ElsaControl.Healing.Core.Tests.csproj
dotnet test tests/ElsaControl.Healing.Persistence.EntityFrameworkCore.Tests/ElsaControl.Healing.Persistence.EntityFrameworkCore.Tests.csproj
dotnet test tests/ElsaControl.Healing.OpenTelemetry.Tests/ElsaControl.Healing.OpenTelemetry.Tests.csproj
dotnet test tests/ElsaControl.Healing.GitHub.Tests/ElsaControl.Healing.GitHub.Tests.csproj
dotnet test tests/ElsaControl.Healing.Agent.Tests/ElsaControl.Healing.Agent.Tests.csproj
dotnet test tests/ElsaControl.Api.Tests/ElsaControl.Api.Tests.csproj --filter Healing
dotnet build ElsaControl.sln
npm --prefix src/ElsaControl.Console test -- src/features/healing
npm --prefix src/ElsaControl.Console run typecheck
npm --prefix src/ElsaControl.Console run build
git diff --check
```
...
3. OIDC exchange with Elsa Control audience.
