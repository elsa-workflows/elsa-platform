...
**Feature**: 039 Elsa Control Self-Healing
**Implementation status**: In progress; independent review findings and final Elsa Control release gates pending
...
| FR-014 | Elsa Control incident/episode/work-item projections are canonical | Incident projection, webhook, and end-to-end tests |
...
| FR-040 | Provider identity plus Elsa Control permission and confirmation policy | Human command policy, webhook, and API tests |
| FR-041 | PRs remain human-mergeable independently of Elsa Control auto-merge | Merge policy and console tests |
...
| FR-047 | Elsa Control and external trusted deployment observations | Deployment observation service/API tests |
| FR-048 | Elsa Control only observes deployment; no deployment/rollback authority is injected | Boundary and deployment observation tests |
...
| FR-057 | Elsa Control/workspace/application/environment stops | Configuration, coordinator, and API tests |
...
- Published coordinated packages used by Elsa Control: `Elsa.Diagnostics.OpenTelemetry` and `Elsa.Diagnostics.OpenTelemetry.Core` `4.0.0-preview.167`
- Validation: OpenTelemetry 71/71 tests; Foundation solution build succeeded; both packages packed successfully.

### Elsa Control

- Branch: `039-elsa-control-self-healing`
...
| Gate | Result |
| --- | --- |
| Elsa Control solution build | Passed, 0 warnings / 0 errors |
| Elsa Control full .NET suite | Final rerun pending after review corrections |
