...
- `elsa-control` owns the Healing signal profile, client and component-manifest packages, all incident/repair/policy state, OpenTelemetry bridge, GitHub and agent adapters, API, persistence, workers, and Console experience.

## Technical Context

**Language/Version**: C# 14 / .NET 10; TypeScript 5.7 with React 18

**Primary Dependencies**: ASP.NET Core minimal APIs; Entity Framework Core 10; Foundation `Elsa.Diagnostics.OpenTelemetry` packages; `HttpClientFactory`; GitHub Copilot SDK behind the no-tools managed inference seam; React Query; existing Elsa Control deployment and workspace authorization infrastructure
...
**Target platform**: Linux-hosted Elsa Control control plane; .NET and ASP.NET Core monitored applications; GitHub-hosted repositories and GitHub Actions runners
...
**Constraints**: Post-redaction evidence only; workspace isolation; idempotent durable intake; no agent access to Git write credentials; no self-modifying repairs; maximum two automatic attempts by default; Elsa Control observes but never deploys or rolls back
...
|---|---|---|
| I. Control Plane First | Elsa Control owns governance, repair orchestration, provider projections, and verification records. Runtime workflow state, deployment, and rollback remain outside Healing. | PASS |
...
1. **Durable intake before background analysis**: Foundation calls additive contributors after redaction and before telemetry storage/live publication. Elsa Control's contributor performs only an idempotent durable inbox append. Classification, deduplication, attribution, and repair dispatch happen in leased background workers.
2. **Elsa Control incident is canonical**: GitHub issues, labels, comments, workflows, branches, and pull requests are projections or commands. They never become the source of truth.
...
7. **Cross-repository sequencing**: Foundation's generic contribution contract lands first. Elsa Control consumes the published package version and verifies the combined OTLP-to-inbox path with a local-package integration lane before Elsa Control merge.
8. **Independent persistence ownership**: Healing uses a dedicated context, migrations, stores, and migration history. It may share a physical database and workspace/application identifiers with other Elsa Control subsystems without making Package Catalog persistence its owner.
...
```text
specs/039-elsa-control-self-healing/
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
...
```text
src/
├── ElsaControl.Healing.Abstractions/
├── ElsaControl.Healing.Core/
│   ├── Configuration/
│   ├── Incidents/
│   ├── Manifests/
│   ├── Ownership/
...
│   ├── Security/
│   └── Verification/
├── ElsaControl.Healing.Agent/
├── ElsaControl.Healing.GitHub/
├── ElsaControl.Healing.OpenTelemetry/
├── ElsaControl.Healing.Client/
├── ElsaControl.Healing.ComponentManifest/
├── ElsaControl.Healing.ComponentManifest.Generator.MSBuild/
├── ElsaControl.Healing.Persistence.EntityFrameworkCore/
├── ElsaControl.Healing.Persistence.SqliteMigrations/
├── ElsaControl.Healing.Persistence.SqlServerMigrations/
├── ElsaControl.Api/Workspace/Healing/
└── ElsaControl.Console/src/features/healing/

tests/
├── ElsaControl.Healing.Abstractions.Tests/
├── ElsaControl.Healing.Core.Tests/
├── ElsaControl.Healing.Agent.Tests/
├── ElsaControl.Healing.GitHub.Tests/
├── ElsaControl.Healing.OpenTelemetry.Tests/
├── ElsaControl.Healing.Client.Tests/
├── ElsaControl.Healing.ComponentManifest.Tests/
├── ElsaControl.Healing.ComponentManifest.Generator.MSBuild.Tests/
├── ElsaControl.Healing.Persistence.EntityFrameworkCore.Tests/
├── ElsaControl.Api.Tests/Healing/
└── ElsaControl.Console/src/features/healing/*.test.tsx
```
...
**Structure Decision**: Healing receives dependency-light abstractions plus explicit core, persistence, agent, OpenTelemetry bridge, and provider adapters. Existing Elsa Control host projects remain composition roots for API and Console UI. The Foundation change is deliberately a generic telemetry extension point so neither repository reverses its ownership boundary.

## Delivery Gates

1. Foundation contribution ordering/redaction/failure tests pass and the package is available to Elsa Control.
2. Elsa Control contracts, models, and persistence migrations pass SQLite and SQL Server model verification.
...
10. Foundation PR merges and its artifact is consumable before the dependent Elsa Control PR merges.
