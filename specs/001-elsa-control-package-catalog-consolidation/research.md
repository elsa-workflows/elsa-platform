# Research: Elsa Control Package Catalog Consolidation

## Decision: Consolidate Package Catalog Into Elsa Control
...
## Decision: Rename Toward `ElsaControl.*`
...
**Rationale**: NuGet is the first source type, but catalog core should not assume NuGet. Moving NuGet sync into `ElsaControl.PackageCatalog.Sources.NuGet` makes future source types possible without catalog core churn.
...
- Treat these capabilities as `ElsaControl.RuntimeBuilder.*`, not as Package Catalog core.
...
- Deployment may later consume Runtime Builder artifacts or contracts, but live reconciliation remains in `ElsaControl.Deployment.*`.
...
- Map `src/ElsaControl.PackageCatalog.Core/Builder/*` to `ElsaControl.RuntimeBuilder.Core`.
- Map `src/ElsaControl.PackageCatalog.Core/DeploymentTemplates/*` to `ElsaControl.RuntimeBuilder.DeploymentTemplates`.
- Map `src/ElsaControl.PackageCatalog.Core/RuntimeConfigurations/*` and persistence stores to Runtime Builder contracts/persistence unless implementation feedback shows they should remain catalog-owned.
...
- Keep `elsa-control` `.specify/` as the active Spec Kit installation.
...
- These package IDs are not published on nuget.org yet, so renaming to `Elsa.Specifications.PackageManifests` and `Elsa.Specifications.PackageManifest.Generator` is likely safe.
...
- The old repository README was updated on `main` in commit `cf7411d` to mark active development as moved to `https://github.com/valence-works/elsa-control`.
