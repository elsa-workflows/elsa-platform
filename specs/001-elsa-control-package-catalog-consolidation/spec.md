# Feature Specification: Elsa Control Package Catalog Consolidation

**Feature Branch**: `001-elsa-control-package-catalog-consolidation`

**Created**: 2026-05-19

**Status**: Draft

**Input**: User description: "Reimagine the current `elsa-package-catalog` repository as Elsa Control subsystems, move toward the ideal package layout under `elsa-control`, deprecate the old repository, improve architecture where useful, incorporate merged PR #36 Runtime Builder backend foundations, and use Spec Kit for implementation."
...
- Q: Which Spec Kit state wins when repositories conflict? -> A: Keep `elsa-control` Spec Kit infrastructure active and import catalog specs without overwriting `.specify/feature.json`.
...
### User Story 1 - Establish Elsa Control Home (Priority: P1)

Elsa maintainers can see the package catalog, package manifests, and package manifest generator represented as first-class Elsa Control subsystems with clear ownership, naming, boundaries, and migration phases.
...
Elsa maintainers can import the current package catalog work into `elsa-control`, keep existing behavior testable, and avoid mixing mechanical movement with architectural rewrites.
...
**Independent Test**: Verify the platform solution contains target projects named `ElsaControl.PackageCatalog.*`, `Elsa.Specifications.PackageManifests`, and `Elsa.Specifications.PackageManifest.Generator*`, with dependency direction enforced by project references and tests.

**Acceptance Scenarios**:

1. **Given** catalog code has been imported, **When** package names are normalized, **Then** Package Catalog projects use `ElsaControl.PackageCatalog.*`.
...
1. **Given** imported catalog code contains `Builder`, `DeploymentTemplates`, and `RuntimeConfigurations` areas, **When** package boundaries are normalized, **Then** those areas move toward `ElsaControl.RuntimeBuilder.*` projects.
...
Elsa maintainers and contributors know that active package catalog development has moved to `elsa-control`, and the old `elsa-package-catalog` repository remains useful only as historical reference or an archived redirect.
...
- Spec Kit files from the catalog repository conflict with Spec Kit files in `elsa-control`.
...
- **FR-002**: The target source layout MUST include `ElsaControl.PackageCatalog.*`, `Elsa.Specifications.PackageManifests`, and `Elsa.Specifications.PackageManifest.Generator*` projects.
...
- **FR-011**: The plan MUST define compatibility handling for existing `Elsa.Specifications.PackageManifests` and `Elsa.Specifications.PackageManifest.Generator` package identities before renaming published package IDs.
...
- **FR-017**: Runtime Builder backend capabilities introduced by `elsa-package-catalog#36` MUST be mapped to a dedicated `ElsaControl.RuntimeBuilder.*` subsystem during architecture normalization.
...
- **FR-020**: Importing catalog Spec Kit files MUST NOT overwrite the active `elsa-control` `.specify/feature.json` or platform Spec Kit installation.
...
- **Elsa Control Subsystem**: A bounded area within `elsa-control`, such as Deployment or Package Catalog, with source projects, tests, specs, and ownership rules.
...
- **Runtime Builder Subsystem**: Elsa Control subsystem that owns builder intent, runtime image metadata, server-side planning, generated bundles, deployment templates, and saved runtime configurations.
...
- **Deprecation Notice**: Documentation and repository state that redirects contributors from the old catalog repository to `elsa-control`.
...
- **SC-002**: The first implementation phase can be verified by building the imported catalog code and running its existing test suites from `elsa-control`.
