...
**Primary Dependencies**: ASP.NET Core minimal APIs, existing workspace identity/authorization and deployment permissions, EF Core catalog persistence, `Elsa.Platform.Deployment.Core` deployment run services, `Elsa.Platform.Deployment.Artifacts` envelope metadata, xUnit, FluentAssertions, React Router, TanStack Query, and Vitest.

**Storage**: Existing catalog relational database through `Elsa.Platform.PackageCatalog.Persistence.EntityFrameworkCore`, with SQLite and SQL Server migrations. Command tables store command metadata, lease/attempt state, progress, safe diagnostics, artifact/revision references, and runtime result references only.
...
**Target Platform**: ASP.NET Core Platform API and runtime-facing workspace command endpoints.
...
- **Control Plane First**: Pass. Commands represent platform-owned deployment intent and metadata; runtime execution remains outside Platform.
