...
namespace ValenceControl.Healing.Client;

public sealed class HealingClientOptions
{
    public Uri? ControlBaseAddress { get; set; }
    ...
    IOptions<HealingClientOptions> options) : IHealingClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var message = new HttpRequestMessage(HttpMethod.Post, new Uri(settings.ControlBaseAddress!, path))
        {
    ...
    {
        if (settings.ControlBaseAddress is null || !settings.ControlBaseAddress.IsAbsoluteUri ||
            settings.ControlBaseAddress.Scheme != Uri.UriSchemeHttps ||
            settings.WorkspaceId == Guid.Empty || settings.ApplicationId == Guid.Empty || settings.EnvironmentId == Guid.Empty)
            throw new InvalidOperationException("Healing client options require an HTTPS Control address and non-empty scope IDs.");
        return settings;
    }
...
{
    public static IServiceCollection AddValenceControlHealingClient(
