...
public sealed class PlatformGitHubRepositoryAuthorizationResolver(
...
public sealed class PlatformTrustedGitHubPublicationContextResolver(
    HealingDbContext dbContext,
    IGitHubRepositoryAuthorizationResolver authorizationResolver) : ITrustedGitHubPublicationContextResolver
{
    ...
    private static string ComputeEnvelopeDigest(RepairResultEnvelope result) =>
        Elsa.Platform.Healing.Agent.RepairAgentGateway.ComputeSha256Digest(JsonSerializer.Serialize(result));
...
public sealed class PlatformGitHubRepairTargetInspector(
    HealingDbContext dbContext,
