    ...
    ITrustedGitHubRepositoryPublisher repositoryPublisher) : ITrustedPatchPublisher
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var plan = new TrustedGitHubPatchPlan(
            $"elsa-healing/{request.AttemptId:N}",
            request.TargetBranch,
            request.ExpectedTargetRevision,
            $"fix: remediate Elsa Healing incident {request.IncidentId:N}",
            $"[Elsa Healing] {SafeText(request.Result.CausalSummary, 160)}",
    ...
    private static string BuildPullRequestBody(
        ...
        <!-- elsa-healing:attempt:{request.AttemptId:N}:patch:{request.Result.PatchDigest} -->
        ...
        This pull request was published by the trusted Elsa Platform publisher. Repository content and agent output were treated as untrusted data.
        """;
}
