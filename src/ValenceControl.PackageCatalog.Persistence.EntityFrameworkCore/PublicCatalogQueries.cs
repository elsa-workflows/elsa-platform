...
public sealed class PublicCatalogQueries(CatalogDbContext dbContext) : IPublicCatalogQueries
{
    ...
    private static RuntimeCompatibility ResolveRuntimeCompatibility(PackageVersion version)
    {
        ...
        var packageRuntimeKinds = ValenceControl.PackageCatalog.Core.Compatibility.RuntimeKindCompatibilityPolicy.ResolvePackageRuntimeKinds(manifest);
        var featureRuntimeKinds = (manifest?.Features ?? [])
            .ToDictionary(
                feature => feature.Id,
                feature => ValenceControl.PackageCatalog.Core.Compatibility.RuntimeKindCompatibilityPolicy.ResolveFeatureRuntimeKinds(feature, packageRuntimeKinds),
