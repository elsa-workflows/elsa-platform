...
import type { BuilderCatalog, RuntimeBuilderIntent } from "@/features/runtime-builder/runtimeBuilderModels";
...
describe("RuntimeBuilderPage", () => {
  ...
  it("builds a runtime plan and previews generated bundle files", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith(`/api/workspaces/${workspaceId}/builder/plan`)) {
        ...
        return jsonResponse({
          resolved: {
            ...body.intent,
            infrastructure: [{ kind: "Database", providerId: "postgresql", strategy: "Managed", settings: null }]
          },
          autoAdded: {
            packages: [],
            features: [],
    ...
    await waitFor(() => {
      const planCall = fetchMock.mock.calls.find((call) => call[0].toString().endsWith(`/api/workspaces/${workspaceId}/builder/plan`));
