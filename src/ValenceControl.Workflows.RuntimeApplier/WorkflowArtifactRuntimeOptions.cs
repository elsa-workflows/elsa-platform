...
public sealed record WorkflowArtifactRuntimeOptions
{
    ...
    public void Validate()
    {
        ...
        if (WorkspaceId is null || WorkspaceId == Guid.Empty)
            throw new InvalidOperationException("Control workspace is required before runtime command sync can start.");
        if (EngineId is null || EngineId == Guid.Empty)
            throw new InvalidOperationException("Control engine is required before runtime command sync can start.");
        ...
        if (ClaimLeaseDuration < TimeSpan.FromSeconds(1) || ClaimLeaseDuration.TotalSeconds > int.MaxValue)
            throw new InvalidOperationException("Runtime command lease duration must be between 1 second and the maximum supported Control lease.");
