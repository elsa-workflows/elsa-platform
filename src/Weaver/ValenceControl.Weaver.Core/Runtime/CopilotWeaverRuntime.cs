    ...
    ILogger<CopilotWeaverRuntime> logger) : IWeaverRuntime
{
        ...
        return new CopilotClientOptions
        {
            ...
            Telemetry = settings.Telemetry.Enabled
                ? new TelemetryConfig
                {
                    ...
                    SourceName = "ValenceControl.Weaver",
        ...
        where TConfig : SessionConfigBase, new()
    {
        ...
        {
            ClientName = "Valence Control Weaver",
            ...
            OnPermissionRequest = (_, _) => Task.FromResult(PermissionDecision.Reject("Weaver only allows explicitly registered Valence Control tools in this host."))
        };
    ...
    private AIFunctionDeclaration CreateCurrentContextTool(WeaverRuntimeRequest request)
    {
        ...
        return CopilotTool.DefineTool(
            ...
            factoryOptions: new AIFunctionFactoryOptions
            {
                ...
                Description = "Read the current Valence Control workspace and console route context. This tool is read-only."
            });
    }
    ...
    private static string BuildSystemMessage(WeaverRuntimeRequest request) =>
        $"""
        You are Weaver, the Valence Control operations agent for workspace {request.WorkspaceId}.
        Work in {request.Mode} mode.
        Use only Valence Control tools exposed by this host.
        Do not use shell, filesystem, package manager, network, source-control, or generic editing tools.
        For any deployment, rollback, promotion, credential, package, runtime, tenant, or workspace mutation, explain the plan and require Valence Control approval before execution.
