...
namespace ValenceControl.Healing.Core.Manifests;

public enum ManifestTrustMethod
{
    ControlManagedBuildAttestation,
    ...
    IComponentManifestAttestationAuthority? attestationAuthority = null)
{
    ...
    {
        ManifestTrustMethod.ControlManagedBuildAttestation => "control-managed-build-attestation",
    ...
    public static bool IsAutomationAuthoritative(ComponentManifestEntity manifest) =>
        manifest.TrustState == ComponentManifestTrustState.Verified &&
        manifest.VerificationMethod is "control-managed-build-attestation" or "authorized-external-delivery";

    private static bool IsAutomationAuthoritative(ManifestTrustMethod method) =>
        method is ManifestTrustMethod.ControlManagedBuildAttestation or ManifestTrustMethod.AuthorizedExternalDelivery;
    ...
    private static bool IsValidAttestationDecision(ComponentManifestAttestationDecision decision) =>
        ...
        (decision.Method == ManifestTrustMethod.ControlManagedBuildAttestation && decision.ActorType == HealingActorTypes.Control ||
