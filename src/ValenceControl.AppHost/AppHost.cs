...
builder.AddAzureAppServiceEnvironment("valence-control")
...
var api = builder.AddProject<Projects.ValenceControl_Api>("api")
...
{
    const string keycloakAuthority = "https://127.0.0.1:8080/realms/valence-control";
    const string keycloakClientId = "valence-control-console";
    ...
    var keycloakRealmImport = Path.GetFullPath(
        Path.Combine(builder.AppHostDirectory, "../../dev/keycloak/valence-control-realm.json"));
    var keycloak = builder.AddKeycloak(
            "keycloak",
            port: 8080,
            adminUsername: keycloakAdminUsername,
            adminPassword: keycloakAdminPassword)
        .WithDataVolume("valence-control-keycloak-data")
        .WithRealmImport(keycloakRealmImport);
    var console = builder.AddViteApp("console", "../ValenceControl.Console")
        .WithReference(api)
    ...
    api.WithEnvironment("Database__Provider", "Sqlite")
        ...
        .WithEnvironment("ConnectionStrings__Catalog", "Data Source=valence-control-catalog-dev.db")
