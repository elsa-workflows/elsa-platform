...
function renderConsoleLogsPage() {
  mocks.getRecentConsoleLogs.mockResolvedValue({
    ...
    sources: [{ id: "elsa-platform-api", displayName: "Elsa Platform API", health: "Connected" }]
  });
  mocks.listConsoleLogSources.mockResolvedValue([{ id: "elsa-platform-api", displayName: "Elsa Platform API", health: "Connected" }]);
...
function lineFixture(id: string, stream: 0 | 1, text: string) {
  return {
    ...
    stream,
    text,
    source: { id: "elsa-platform-api", displayName: "Elsa Platform API", health: "Connected" },
    truncated: false
  };
}
