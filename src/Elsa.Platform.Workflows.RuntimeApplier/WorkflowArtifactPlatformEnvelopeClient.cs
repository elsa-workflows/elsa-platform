...
public sealed class WorkflowArtifactPlatformEnvelopeClient(
    ...
    WorkflowArtifactRuntimeOptions options) : IWorkflowArtifactEnvelopeProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var body = await ReadJsonAsync<WorkspaceArtifactDto>(response, cancellationToken)
            ?? throw new InvalidOperationException("Platform artifact response could not be read.");
        if (body.WorkspaceId != _options.WorkspaceId!.Value)
            throw new InvalidOperationException("Platform artifact response workspace does not match the configured runtime workspace.");
        if (body.Id != artifact.ArtifactRecordId.Value)
            throw new InvalidOperationException("Platform artifact response does not match the requested artifact record.");
