...
describe("DeploymentsPage", () => {
  ...
  it("creates application setup with environments and engines from the guided setup route", async () => {
    ...
    await userEvent.type(screen.getByLabelText("Key Vault reference"), "kv://acme-valence-control/prod/claims-engine-api");
    ...
    await waitFor(() =>
      expect(requestBody(fetchMock, "POST", "/credential-references")).toMatchObject({
        ...
        reference: "kv://acme-valence-control/prod/claims-engine-api",
        secretValue: null
      })
    );
    await waitFor(() =>
  ...
  it("renders standalone engine credential management for the selected workspace", async () => {
    ...
    expect(screen.getByText("Engine credentials let Valence Control interact with registered workflow engines. Runtime secrets and artifact secret references stay in the runtimes.")).toBeInTheDocument();
  ...
  it("edits credential references from a dedicated edit page", async () => {
    ...
    expect(requestBody(fetchMock, "PUT", "/credential-references/credential-reference-dev")).toMatchObject({
      ...
      reference: "kv://acme-valence-control/dev/elsa-api"
    });
  });
  ...
  it("keeps assistant plans immutable and scoped to environment detail", async () => {
    ...
    expect(screen.getByText("No Valence Control mutations executed.")).toBeInTheDocument();
...
const deploymentCredentialReferences: WorkspaceDeploymentCredentialReference[] = [
  {
    ...deploymentCredentialReference(
      ...
      "Azure Key Vault",
      "Dev engine API",
      "kv://acme-valence-control/dev/elsa-api",
  ...
  deploymentCredentialReference(
    ...
    "Azure Key Vault",
    "Prod engine API",
    "kv://acme-valence-control/prod/elsa-api",
  ...
  comparisons: [
    {
      ...
      diff: [
        ...
        { id: "secret-payment-api", category: "SecretReferences", name: "Payment API", sourceValue: "kv://acme-valence-control/prod/payment-api:v3", targetValue: "Missing reference", impact: "Changed" }
      ],
      validations: [
    ...
    },
    {
      ...
      diff: [
        ...
        { id: "secret-payment-test", category: "SecretReferences", name: "Payment API", sourceValue: "kv://acme-valence-control/test/payment-api:v3", targetValue: "kv://acme-valence-control/test/payment-api:v2", impact: "Changed" }
      ],
      validations: [
...
function engine(
  ...
  return {
    ...
    credentialReference: {
      ...
      reference: `kv://acme-valence-control/${environmentId.replace("claims-", "")}/elsa-api`,
      verificationStatus,
