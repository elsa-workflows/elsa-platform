...
export type WorkspaceArtifactUploadCapabilities = {
...
export type CreateArtifactUploadResponse = {
  uploadId: string;
  status: "Pending" | "Uploading" | "Uploaded" | "Inspecting" | "Completed" | "Failed" | "Aborted" | "Expired";
  expiresAt: string;
  maxUploadBytes: number;
};
...
export type CompleteArtifactUploadResponse = {
  uploadId: string;
  status: CreateArtifactUploadResponse["status"];
...
export function artifactDigest(artifact: WorkspaceArtifact) {
  return `${artifact.contentDigest.algorithm}:${artifact.contentDigest.value}`;
}

export function artifactStatusLabel(status: WorkspaceArtifactLifecycleStatus) {
  return status === "Archived" ? "Archived" : "Active";
}

export function artifactFormatLabel(format: WorkspaceArtifactFormat) {
  return format === "Zip" ? "ZIP" : format;
}

export function artifactDiagnosticCount(artifact: WorkspaceArtifact, severity: WorkspaceArtifactDiagnosticSeverity) {
  return artifact.diagnostics.filter((diagnostic) => diagnostic.severity === severity).length;
}
