...
using ElsaControl.RuntimeBuilder.Abstractions.Planner;
...
public static class BuilderEndpoints
{
    public static IEndpointRouteBuilder MapBuilderEndpoints(this IEndpointRouteBuilder endpoints)
    {
            ...
            CancellationToken cancellationToken) =>
        {
            if (request.Intent is null)
                return BuilderProblem(
                    httpContext,
                    "urn:elsa-control:problem:builder-plan-invalid-request",
            ...
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
                ...
                return BuilderProblem(
                    httpContext,
                    "urn:elsa-control:problem:builder-plan-failed",
            ...
            CancellationToken cancellationToken) =>
        {
            if (!TryMapIntent(request, out var intent, out var error))
                return BuilderProblem(
                    httpContext,
                    "urn:elsa-control:problem:builder-bundle-invalid-request",
            ...
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
                ...
                return BuilderProblem(
                    httpContext,
                    "urn:elsa-control:problem:builder-bundle-generation-failed",
    ...
    internal static IResult PlanningTimeoutProblem(BuilderPlanningTimeoutException exception, HttpContext httpContext) =>
        Results.Problem(
            type: "urn:elsa-control:problem:builder-plan-timeout",
