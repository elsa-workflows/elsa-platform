...
public sealed class StudioPlatformArtifactSubmitClient(HttpClient httpClient) : IStudioPlatformRevisionSubmitClient
{
    ...
    private static void ValidateRevisionOptions(StudioSubmitOptions options)
    {
        if (options.ApplicationId is null || options.ApplicationId == Guid.Empty)
            throw new InvalidOperationException("Platform application is required before creating a desired-state revision.");
        if (options.EnvironmentId is null || options.EnvironmentId == Guid.Empty)
            throw new InvalidOperationException("Platform environment is required before creating a desired-state revision.");
    }
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"elsa-studio-platform-submit-{Guid.NewGuid():N}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (created is null)
            return ArtifactUploadResult.Failed(new StudioSubmitResult(StudioSubmitStatus.RetryableError, "Platform artifact upload response could not be read."));
        ...
        if (completed?.Artifact is null)
            return ArtifactUploadResult.Failed(new StudioSubmitResult(StudioSubmitStatus.RetryableError, "Platform artifact completion response could not be read."));
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (revision is null)
            return new StudioSubmitResult(StudioSubmitStatus.RetryableError, "Platform revision response could not be read.");

        return new StudioSubmitResult(
            StudioSubmitStatus.Submitted,
            "Submitted to Platform desired state.",
            artifact.ArtifactId,
    ...
    private static void TryDeleteBuildDirectory(string zipPath)
    {
        ...
        {
            // Best-effort cleanup only; callers should not fail after Platform accepted a submission.
        }
    }
    ...
    private sealed record ArtifactUploadResult(WorkspaceArtifactResponse? Artifact, StudioSubmitResult Result)
    {
        ...
        public static ArtifactUploadResult Completed(WorkspaceArtifactResponse artifact) =>
            new(artifact, new StudioSubmitResult(StudioSubmitStatus.Submitted, "Artifact uploaded to Platform."));
