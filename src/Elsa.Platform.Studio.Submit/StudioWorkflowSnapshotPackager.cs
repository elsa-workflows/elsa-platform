    ...
    ArtifactEnvelopeValidator? validator = null) : IStudioWorkflowSnapshotPackager
{
        ...
        DateTimeOffset? packagedAt = null)
    {
        ...
        var contentDigest = ComputeDigest(snapshot.WorkflowDefinitionJson);
        ...
        var payloadReference = new ArtifactPayloadReference(
            ...
            Encoding.UTF8.GetByteCount(snapshot.WorkflowDefinitionJson),
            contentDigest);
        ...
        return new StudioSubmitPackage(envelope, snapshot.WorkflowDefinitionJson, (packagedAt ?? DateTimeOffset.UtcNow).ToUniversalTime());
    }
    ...
    private static void ValidateOptions(StudioSubmitOptions options)
    {
        ...
        if (options.AuthenticationMode == StudioSubmitAuthenticationMode.ProviderCredentialReference && string.IsNullOrWhiteSpace(options.CredentialReference))
            throw new InvalidOperationException("Platform credential reference is required for provider-backed Studio submission.");
