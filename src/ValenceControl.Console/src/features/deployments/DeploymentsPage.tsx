...
const secretStoreTypeOptions: { value: DeploymentSecretStoreType; label: string; description: string; referenceLabel: string; referencePlaceholder: string }[] = [
  {
    ...
    description: "Valence Control stores protected engine credential material.",
  ...
  },
  {
    ...
    description: "Valence Control stores a Key Vault locator only.",
    referenceLabel: "Key Vault reference",
    referencePlaceholder: "kv://valence-control/dev/engine-api"
  },
  {
    value: "KubernetesSecrets",
    label: "Kubernetes Secrets",
    description: "Valence Control stores a namespace/name/key locator only.",
  ...
  },
  {
    ...
    description: "Valence Control stores an engine-host environment variable name only.",
  ...
  },
  {
    ...
    description: "Valence Control stores a customer-governed reference it cannot browse or verify.",
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="rounded-ui border border-border bg-muted/30 p-3 text-sm text-muted-foreground">
        Engine credentials let Valence Control interact with registered workflow engines. Runtime secrets and artifact secret references stay in the runtimes.
      </div>
      ...
      <section className="space-y-3">
        <SectionHeader title="Credential stores" description="Workspace-scoped stores that hold or locate credentials used by Valence Control to call engines." />
...
function DeploymentCredentialStoreCreateReady({ context }: { context: DeploymentContext }) {
  ...
  return (
    <FormPageShell
      ...
      description="Create a workspace-scoped store for credentials Valence Control uses to call registered engines."
      breadcrumbs={[
        { label: "Deployments", to: "/admin/deployments" },
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.45fr)]">
        ...
        <div className="space-y-4">
          ...
          {reference.status === "Active" ? (
            <div className="rounded-ui border border-border bg-surface p-4">
              <SectionHeader title="Archive reference" description="Archive only after reassignment or after confirming affected engines no longer need Valence Control management." />
              {confirmArchive ? (
                <div className="mt-3 space-y-3">
        ...
        <div className="rounded-ui border border-border bg-surface">
          ...
          {usageReference ? (
            <div className="border-t border-border px-3 py-3">
              ...
              <p className="mt-2 text-xs text-muted-foreground">Archive only after reassignment or after confirming the affected engines no longer need Valence Control management.</p>
            </div>
          ) : null}
        </div>
      ...
      <Panel title={`Assistant plan ${plan.id} v${plan.version}`} icon={<Bot className="h-4 w-4" />}>
        <div className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <ActionList title="Executed actions" actions={plan.executedActions.length > 0 ? plan.executedActions : ["No Valence Control mutations executed."]} icon={<ShieldCheck className="h-4 w-4 text-success" />} muted={plan.executedActions.length === 0} />
          </div>
