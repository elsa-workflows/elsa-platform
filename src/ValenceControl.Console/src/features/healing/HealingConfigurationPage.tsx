...
export function HealingLandingPage() {
  return (
    <EmptyState
      ...
      description="Review normalized incidents, or open a deployed application to configure exception discovery and approved component ownership. Automatic discovery also requires the Control OpenTelemetry module."
      ...
      {canConfigure ? (
        <div className="grid gap-4 xl:grid-cols-2">
          ...
          {canApproveOwnership ? <form className="space-y-3 rounded-ui border border-border bg-surface p-4" onSubmit={(event) => { event.preventDefault(); if (authorityDraft.automaticMergeEnabled) setConfirmAuthorityAutoMerge(true); else createAuthority.mutate(); }}>
            ...
            <p className="text-xs text-muted-foreground">Connect a GitHub App installation to a protected workspace credential. Store the write-only credential as JSON containing appId and privateKeyPem. The connection remains pending and cannot authorize repair until Control validates the installation and repository identity.</p>
            <div className="grid gap-3 sm:grid-cols-2">
