# Valence Control Console

Unified React console for Valence Control workspace users and operators. Package
...
are reserved as Valence Control modules from the beginning.
...
The Vite dev server proxies relative `/api` requests to the Valence Control API
...
When running `src/ValenceControl.AppHost`, Aspire starts this Vite app as the
...
and the Playwright smoke test in `tests/ValenceControl.Console.E2E/package-details.spec.ts`.
