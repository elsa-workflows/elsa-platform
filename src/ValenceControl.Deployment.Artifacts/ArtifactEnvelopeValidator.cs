...
public sealed partial class ArtifactEnvelopeValidator(IArtifactTypeRegistry typeRegistry)
{
    ...
    public void Validate(ArtifactEnvelope envelope)
    {
        if (string.IsNullOrWhiteSpace(envelope.ArtifactId))
            throw new InvalidOperationException("Artifact identity is required.");
        if (!envelope.EnvelopeVersion.Equals(ArtifactEnvelopeConstants.EnvelopeVersion, StringComparison.Ordinal))
            throw new InvalidOperationException("Artifact envelope version is not supported.");
        var artifactType = typeRegistry.FindType(envelope.ArtifactTypeId)
            ?? throw new InvalidOperationException("Artifact type is not supported.");
        if (!artifactType.Enabled)
            throw new InvalidOperationException("Artifact type is not enabled.");
        if (!artifactType.SupportedSchemaVersions.Contains(envelope.ArtifactSchemaVersion, StringComparer.OrdinalIgnoreCase))
        ...
        if (envelope.ManifestDigest is { } manifestDigest)
            ValidateDigest(manifestDigest.Algorithm, manifestDigest.Value, "Artifact manifest digest");
        ...
        foreach (var diagnostic in envelope.Diagnostics)
            ValidateSafeValue(diagnostic.Message, "Artifact diagnostic message");
    }
    ...
    public bool ContainsUnsafeText(string? value) =>
        !string.IsNullOrWhiteSpace(value) && UnsafeTerms.Any(term => value.Contains(term, StringComparison.OrdinalIgnoreCase));

    private static void ValidateDigest(string algorithm, string value, string label)
    {
        if (!algorithm.Equals(ArtifactLayoutConstants.ChecksumAlgorithm, StringComparison.OrdinalIgnoreCase) || string.IsNullOrWhiteSpace(value))
            throw new InvalidOperationException($"{label} is required.");
        if (!DigestValueRegex().IsMatch(value))
            throw new InvalidOperationException($"{label} has an invalid digest value.");
    }

    private void ValidatePayloadReference(ArtifactPayloadReference reference)
    {
        ...
        if (reference.ReferenceDigest is { } referenceDigest)
            ValidateDigest(referenceDigest.Algorithm, referenceDigest.Value, "Artifact reference digest");
    }

    private void ValidateProducer(ArtifactProducer producer)
    {
    ...
    private void ValidateDisplayMetadata(ArtifactDisplayMetadata metadata)
    {
    ...
    private void ValidateCompatibilityHints(string artifactTypeId, IReadOnlyList<ArtifactCompatibilityHint> hints)
    {
    ...
    [GeneratedRegex("^[A-Za-z0-9:_\\-.]{3,256}$")]
