...
/// <summary>
/// Authenticates a Platform-issued OTLP source token and establishes server-owned Healing routing claims.
/// Every rejection is deliberately indistinguishable to callers.
/// </summary>
public sealed class PlatformHealingOtlpRequestAuthenticator(
    ...
    HealingTelemetrySourceTokenService tokens) : IOtlpRequestAuthenticator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                ["elsa.platform.telemetry-source.credential-version"] = source.CredentialVersion.ToString(System.Globalization.CultureInfo.InvariantCulture)
            };
            var context = OpenTelemetryIngestionContext.Authenticated(
                $"platform-otel-source:{source.Id:D}", claims, metadata);
