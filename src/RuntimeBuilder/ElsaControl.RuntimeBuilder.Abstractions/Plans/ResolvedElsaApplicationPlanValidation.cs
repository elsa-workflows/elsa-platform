...
public static class ResolvedElsaApplicationPlanValidator
{
    public static IReadOnlyList<ResolvedPlanValidationFinding> Validate(ResolvedElsaApplicationPlan? plan)
    {
        ...
        if (plan.Topology.Components is null || plan.Topology.Components.Count == 0)
            findings.Add(new("topology.components.required", "At least one topology component is required.", "topology.components"));
        Duplicate(plan.Topology.Components, x => x.Id, "topology.component.duplicate", "topology.components");
        foreach (var component in plan.Topology.Components ?? [])
        {
            ...
            if (component.CompanionComponentId is not null && !(plan.Topology.Components ?? []).Any(x => string.Equals(x.Id, component.CompanionComponentId, StringComparison.OrdinalIgnoreCase)))
        ...
        foreach (var capacity in plan.Capacity.Components ?? [])
        {
            ...
            if (!(plan.Topology.Components ?? []).Any(x => string.Equals(x.Id, capacity.ComponentId, StringComparison.OrdinalIgnoreCase)))
        ...
        {
            if (!(plan.Topology.Components ?? []).Any(x => string.Equals(x.Id, endpoint.ComponentId, StringComparison.OrdinalIgnoreCase)))
        ...
        {
            foreach (var group in (items ?? []).GroupBy(keySelector, StringComparer.OrdinalIgnoreCase).Where(x => x.Count() > 1))
    ...
    private static void ValidateImage(ResolvedImageIdentity? image, List<ResolvedPlanValidationFinding> findings, string scope)
    {
        ...
        foreach (var (platform, digest) in image.PlatformDigests ?? new Dictionary<string, string>())
        {
