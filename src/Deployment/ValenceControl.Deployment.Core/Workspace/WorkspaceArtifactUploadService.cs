    ...
    TimeProvider? timeProvider = null)
{
        ...
        try
        {
            ...
            var build = await artifactBuilder.BuildZipAsync(
                new DeploymentArtifactBuildOptions(
                    ...
                    builder: "Valence Control Console",
        ...
        Guid actorAccountId)
    {
        ...
        return new RegisterWorkspaceArtifactRequest(
            ...
            manifestDigest,
            payloadReference,
            new ArtifactProducer("upload", "Valence Control Console", null, metadata.Source),
            displayMetadata,
    ...
    private string LocalStorageRoot =>
        Path.GetFullPath(string.IsNullOrWhiteSpace(_options.LocalStorageRoot)
            ? Path.Combine(Path.GetTempPath(), "valence-control", "artifact-uploads")
