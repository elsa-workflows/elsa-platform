    ...
    TimeProvider? timeProvider = null)
{
        ...
        Guid actorAccountId)
    {
        ...
        return new RegisterWorkspaceArtifactRequest(
            ...
            manifestDigest,
            payloadReference,
            new ArtifactProducer("upload", "Elsa Control Console", null, metadata.Source),
            displayMetadata,
    ...
    private string LocalStorageRoot =>
        Path.GetFullPath(string.IsNullOrWhiteSpace(_options.LocalStorageRoot)
            ? Path.Combine(Path.GetTempPath(), "elsa-control", "artifact-uploads")
