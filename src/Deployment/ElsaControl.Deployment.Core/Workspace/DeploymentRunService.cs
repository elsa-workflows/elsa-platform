    ...
    IArtifactTypeRegistry? artifactTypes = null)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await ValidateDeployabilityAsync(workspaceId, request, cancellationToken);
    ...
    private async Task ValidateDeployabilityAsync(
        Guid workspaceId,
        WorkspaceDeploymentRunRequest request,
        CancellationToken cancellationToken)
    {
        ...
        var deployability = await new DeploymentDeployabilityService(deploymentStore, artifactStore, _artifactTypes, _timeProvider)
            .EvaluateAsync(
                workspaceId,
                request.SourceRevisionId,
                new DeploymentDeployabilityRequest(request.TargetEnvironmentId, request.TargetEngineId, request.Mode),
                cancellationToken);
        if (deployability.Status == DeploymentDeployabilityStatus.Blocked)
            throw new InvalidOperationException(deployability.Blockers.FirstOrDefault()?.Message ?? "Revision is not deployable to the target engine.");
    }
}
