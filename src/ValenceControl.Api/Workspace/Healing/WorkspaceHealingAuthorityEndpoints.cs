...
public sealed class WorkspaceHealingAuthorityEndpointModule : IHealingEndpointModule
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var memberExists = await catalogDbContext.WorkspaceMemberships.AsNoTracking().AnyAsync(
            x => x.WorkspaceId == workspaceId && x.AccountId == request.ControlAccountId,
            cancellationToken);
        ...
        else
        {
            ...
            existing.ControlAccountId = request.ControlAccountId;
...
public sealed record ProviderActorIdentityLinkRequest(string ProviderActorLogin, Guid ControlAccountId);
