    ...
    ArtifactEnvelopeValidator? envelopeValidator = null)
{
    ...
    private static ArtifactEnvelope ToEnvelope(RegisterWorkspaceArtifactRequest request) =>
        ArtifactEnvelopeDefaults.CreateEnvelope(
            ...
            new ValenceControl.Deployment.Abstractions.Artifacts.ArtifactDigest(request.ContentDigest.Algorithm, request.ContentDigest.Value),
            request.ManifestDigest is null ? null : new ValenceControl.Deployment.Abstractions.Artifacts.ArtifactDigest(request.ManifestDigest.Algorithm, request.ManifestDigest.Value),
